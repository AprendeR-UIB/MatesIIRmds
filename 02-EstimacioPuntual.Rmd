...
Recordau que $E(X)=p_X$ i $\sigma_X=\sqrt{p_X(1-p_X)}$, i que si la mostra és aleatòria simple, aquesta $S$ és una variable aleatòria binomial $B(n,p)$.
```
...
```{theorem, pX}
Si $X$ és una variable aleatòria Bernoulli amb probabilitat d'èxit $p_X$, aleshores
...
$$
té distribució coneguda: $\chi_{n-1}^2$ (llegit ``khi quadrat amb $n-1$ graus de llibertat).
...
$$
Per tant, fixat $y_0$, podem escollir $x_1,\ldots,x_{n-1}$ com vulguem i aleshores $x_n$ queda fixat com una de les dues solucions d'aquesta equació de segon grau.
...
* $t_{n,0.95}$ (per a $n\geq 10$) està entre 1.64 i 1.8; ho aproximarem $t_{n,0.95}\approx 1.7$

* $t_{n,0.975}$  (per a $n\geq 10$) està entre 1.96 i 2.2; ho aproximarem $t_{n,0.95}\approx 2$
...
```{example}
Continuem amb l'Exemple \@ref(exm:taxi1). Emprant la fórmula anterior, estimo que el nombre de taxis de Palma era
