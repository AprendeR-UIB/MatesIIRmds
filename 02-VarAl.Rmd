...
\end{array}
$$
perquè, com que  $(x_0,x]\cap D_X=\emptyset$, $P(x_0<X\leqslant x)=0$.
...
* $F_X$ és **creixent**, perquè si $x\leqslant y$, tots els subjectes de $X\leqslant x$ també pertanyen a $X\leqslant y$, i per tant
$$
...
```{block2,type="rmdexercici"}
Sigui $X$ una variable aleatòria discreta uniforme amb domini $D_X=\{1,\ldots,N\}$. Què val el seu valor esperat?
```
...
```{block2,type="rmdexercici"}
Sigui $X$ una variable aleatòria discreta uniforme amb domini $D_X=\{1,\ldots,N\}$. Què val la seva desviació típica?
```
...
```{block2,type="rmdnote"}
Si existeix algun $x_p\in D_X$ tal que $P(X\leqslant x_p)=p$, llavors el $p$-quantil és aquest $x_p$, perquè, per a tot un altre $x\in D_x$:
...
```{block2,type="rmdexercici"}
Sigui $X$ una variable aleatòria discreta uniforme amb domini $D_X=\{1,\ldots,N\}$. Què val la seva mediana?
```
...
* $P(a\leqslant X\leqslant b)=P(X\leqslant b)-P(X<a)$ és l'àrea sota la corba $y=f_X(x)$ a l'esquerra de $x=b$ **menys** l'àrea sota la corba $y=f_X(x)$ a l'esquerra de $x=a$. Per tant, $P(a\leqslant X\leqslant b)$ és igual a **l'àrea sota la corba $y=f_X(x)$ entre $x=a$ i $x=b$.**
...
En el cas concret de la normal estàndard $Z$, per a qualsevol $z\geqslant 0$ es té que les àrees a l'esquerra de $-z$ i a la dreta de $z$ són iguals
$$
...
Per tant $P(X\leqslant 1.037)=0.95$. Per tant
$P(Y\leqslant e^{1.037})=0.95$. Com que $e^{1.037}=2.82$, concloem que  2.82 cc de digitalina són suficients per matar el 95% dels ratolins.
