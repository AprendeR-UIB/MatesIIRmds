...
Dues variables aleatòries $X,Y$ definides sobre una mateixa població són **independents** quan, per a tots els parells de valors $a,b\in \mathbb{R}$, els esdeveniments
$$
...
Més en general, unes variables aleatòries $X_1,X_2,\ldots,X_n$ definides sobre una mateixa població  són **independents** quan, per a tots $a_1,a_2,\ldots,a_n\in \mathbb{R}$, els esdeveniments
$$
...
$$
és la mitjana dels valors de $X$ sobre tots els subjectes de $\Omega$. Comparau-ho amb l'exemple següent.
...
Sí, ja sabem que parlar de variables constants és un oxímoron, però de vegades una variable aleatòria pren el mateix valor sobre tots els subjectes d'una població.
```
...
$$
És a dir, és el valor esperat del quadrat de la diferència entre $X$ i la seva mitjana $\mu_X$. També la indicarem amb $\sigma_X^2$.
...
1. Dóna la seva funció de densitat.
1. Dóna la seva funció de distribució.
1. Si llançàssim moltes vegades els dos daus, què hauríem d'esperar obtenir de mitjana com a suma de les dues cares?
1. Sigui $Y$ la variable aleatòria consistent en llançar un dau cúbic equilibrat i anotar el resultat de la cara que queda en l'aire. És el valor esperat de $X$ el doble del valor esperat de $Y$? Hi ha qualque motiu teòric pel qual ho hagi de ser?
1. Calcula la variància de $X$ i la de $Y$. Hi ha qualque relació numérica visible entre elles? Hi ha qualque motiu teòric que faci que aquesta relació existeixi?
1. Què val el primer quartil de $X$? Quin és el seu significat?
1. Què val la mediana de $X$? Quin és el seu significat?
1. És la mediana de $X$ el doble de la mediana de $Y$? Hi ha qualque motiu teòric pel qual ho hagi de ser?
...
Tenim un subconjunt $E$ d'una població $\Omega$ (per exemple, $\Omega$ els estudiants de la UIB i $E$ els que han aprovat totes les assignatures del primer semestre). Sigui $p$ la proporció poblacional d'individus de la població que pertanyen a $E$, és a dir $p=P(E)$. Prenem **mostres aleatòries simples** de mida $n$ de la població i comptam quants subjectes de la mostra són de $E$. Aquesta variable aleatòria és **binomial** $B(n,p)$.
```
...
1. Si un estudiant respon totes les preguntes a l'atzar i  cada pregunta encertada val 1 punt, quina nota esperes que tregui?

1. Cada pregunta encertada val 1 punt. Què hauria de restar cada resposta equivocada perquè la nota esperada d'un estudiant que respongués totes les preguntes a l'atzar fos un 0?

1. Suposem que les respostes correctes sumen 1, les equivocades resten el que hagis decidit a l'apartat anterior, i les no contestades no sumen ni resten res. Escriu una funció de R que simuli un estudiant que davant cada pregunta de l'examen la deixa en blanc o escull a l'atzar una de les respostes, amb les 6 opcions (en blanc i les 5 respostes) totes amb la mateixa probabilitat 1/6, i doni la nota que obté. 
...
```{block2,type="rmdnote"}
Un exemple extrem que mostra que si permetem repeticions els resultats són, en principi, més variats és el següent. Considerau una població de $P$ individus i en preneu una mostra aleatòria de mida aquest mateix $P$. Si no permeteu repeticions, només n'hi ha una: no teniu més remei que triar la població sencera. Però si permeteu repeticions n'hi ha moltíssimes.
```
...
* A Espanya hi ha 46,700,000 persones, de les quals un 11.7% són diabètics. Triam 100 espanyols diferents i comptam quants són diabètics. 
    ...
    Aquesta variable és, en realitat, hipergeomètrica amb (si les dades donades són exactes) $N=0.117\cdot 46700000=5463900$, $M=46700000-N=41236100$ i $n=100$, però en la pràctica la consideram binomial $B(100,0.117)$. El quocient $(\mathbf{P}-n)/(\mathbf{P}-1)$ és
$$
...
```{block2,type="rmdnote"}
I com que el nombre d'espanyols i la proporció de diabètics són només aproximats, no perdem res si aproximam un poquet més i consideram la variable com a binomial.
```
...
```{block2,type="rmdrecordau"}
Però $P(X=a)=P(X=b)=0$, així que, per favor, evitau dir que "la probabilitat que $X$ valgui $b$ **és 5 vegades més gran** que la probabilitat que valgui $a$". Sí, ja sabem que $5\cdot 0=0$, però la frase és enganyosa: la probabilitat que $X$ valgui $b$ no és més gran que la probabilitat que valgui $a$.
```
...
```{block2,type="rmdrecordau"}
Sigui $X$ una variable discreta que només pot prendre com a valors nombres naturals, com ara una binomial o  una Poisson. 
Quan l'aproximam per mitjà d'una variable normal $Y$, és convenient aplicar alguna **correcció de continuïtat**. La més senzilla és, per a cada $n\in \mathbb{N}$, aproximar:
...
```{block2,type="rmdrecordau"}
L'argument que hem desenvolupat a la nota anterior mostra en general que si $X$ és $N(\mu,\sigma)$,  **si el seu $q$-quantil és $\mu-x$, aleshores el seu $(1-q)$-quantil és $\mu+x$**. 
```
...
Per tant $P(X\leqslant 1.037)=0.95$. D'aquí deduïm que $P(Y\leqslant e^{1.037})=0.95$. Com que $e^{1.037}=2.82$, concloem que  2.82 cc de digitalina són letals pel 95% dels ratolins.
...
1. Llançam un dau cúbic, llançam  una moneda el nombre de vegades que ens hagi sortit al dau, i comptam el nombre de cares. 
...
1. 0.5
1. 0 
1. Depèn de la variable aleatòria, però és estrictament més gran que tots els altres valors de $P(X=x)$
...
1.  El temps al voltant del qual tarda més sovint és 17 segons.
1.  En un 95% de les ocasions tarda aproximadament entre 14 i 20 segons.
1.  Tarda més de 20 segons amb la mateixa freqüència amb la qual tarda menys de 14 segons.
1.  Tarda més de 20 segons amb la mateixa freqüència amb la qual tarda menys de 20 segons.
1.  En un 95% de les ocasions tarda 23 segons o menys.
...
1.  $P(X< 0)=0.5$
1.  $P(X\leqslant -1)=0.2$
1.  $P(X\leqslant 1)=0.2$
1.  $P(X\leqslant 1)=0.8$
...
1. Si preneu les mostres sense permetre individus repetits, els resultats surten menys variats que si les preneu permetent repeticions. 
...
1. Si preneu les mostres permetent que hi surtin individus repetits, com més grans preneu les mostres menys variats surten els resultats.
...
1. Si preneu les mostres sense permetre individus repetits, com més grans preneu les mostres menys variats surten els resultats.
1. Cap de les altres afirmacions és vertadera.
