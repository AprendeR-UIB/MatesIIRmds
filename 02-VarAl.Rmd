...
```{example}
Seguim amb la variable aleatòria $X$ "Llançam una moneda equilibrada 3 vegades i comptam les cares". La seva esperança és
$$
...
* Les seves propietats bàsiques, com ara el seu valor esperat i si la densitat és simètrica o té una cua a qualque costat.
...
* Realitzar un test PCR de COVID-19 a una persona i mirar si dóna positiu
    ...
    * $E$: donar positiu
    * $p$: la proporció de persones que donen positiu en el test a la població de la qual hem extret el nostre subjecte
...
Ara, quantes paraules hi ha amb $k$ $E$'s i $n-k$ $F$'s? Cada una d'elles queda caracteritzada per les posicions de les $k$ $E$'s, per tant és el nombre de possibles eleccions de conjunts de $k$ posicions per a les $E$'s. Això darrer és el nombre de possibles subconjunts de $k$ elements (les posicions on hi haurà les $E$'s) de $\{1,\ldots,n\}$, que és el nombre combinatori $\binom{n}{k}$.
Per tant ja tenim
$$
...
* A Espanya hi ha 46,700,000 persones, de les quals un 11.7% són diabètics. Triam 100 espanyols diferents a l'atzar (de manera independent els uns dels altres) i comptam quants són diabètics.
     ...
     No és binomial, pel mateix motiu que no ho era quan elegíem estudiants sense permetre repeticions. Però **pràcticament** sí que ho és, perquè les probabilitats gairebé no varien d'una elecció a la següent. 
     
     Per exemple, quan ja duim 99 individus escollits, la probabilitat de triar un individu concret dels que queden és $1/(46700000-99)=2.141332\times 10^{-8}$ mentre que si permetem repeticions, aquesta probabilitat és $1/46700000=2.141328\times 10^{-8}$. Conicdeixen fins la dotzena xifra decimal.
...
Si $p=0.5$, la funció densitat és simètrica respecte de $n/2$: com que $E$ i $F$ tenen la mateixa probabilitat, 0.5, la probabilitat de treure $k$ $E$'s és la mateixa que la de treure $k$ $F$'s, és a dir, la de treure $n-k$ $E$'s.
...
En canvi, si $p\neq 0.5$, la funció densitat no és simètrica, com podeu veure als gràfics de més a dalt. 
...
* Si llançam 20 vegades un dau cúbic equilibrat, quina és la probabilitat de treure com a màxim 5 uns? 
    ...
    Amb les notacions anteriors, ens demanen $P(X\leq 5)$, i aquesta probabilitat ens la dóna la funció de distribució de $X$. És $F_X(5)$:
```{r}
pbinom(5,20,1/6)
```

* Si llançam 20 vegades un dau cúbic equilibrat, quina és la probabilitat de treure menys de 5 uns? 

    Amb les notacions anteriors, ens demanen $P(X< 5)$, és a dir, $P(X\leq 4)=F_X(4)$:
```{r}
pbinom(4,20,1/6)
```
...
* Si llançam 20 vegades un dau cúbic equilibrat, quina és la probabilitat de treure 8 uns o més? 
    ...
    Amb les notacions anteriors, ens demanen $P(X\geq 8)$. Com que el contrari de treure 8 uns o més és treure 7 uns o menys, tenim que $P(X\geq 8)=1-P(X\leq 7)=1-F_X(7)$:
```{r}
1-pbinom(7,20,1/6)
```
...
```{block2,type="rmdcorbes"}
La demostració de la fórmula per a la densitat és senzilla, en termes de casos favorables partit per casos possibles. Vegem: $f_X(k)$ és la probabilitat que un subconjunt de $n$ subjectes (diferents) de la població contingui $k$ subjectes  $E$ i $n-k$ subjectes dels altres (en direm $F$).
...
Vegem alguns gràfics de la funció densitat de variables aleatòries de Poisson.
...
```{block2,type="rmdexercici"}
El nombre d'estudiants que entren en el bar de l'edifici en un interval de 5 minuts, creieu que segueix una distribució de Poisson?
```
...
* Si $\varepsilon>0$ és molt, molt petit, l'àrea sota la corba $y=f_X(x)$ entre $a-\varepsilon$ i $a+\varepsilon$ és aproximadament igual a la del rectangle de base l'interval $[a-\varepsilon,a+\varepsilon]$ i alçada $f_X(a)$, és a dir, a $2\varepsilon\cdot f_X(a)$ (vegeu la Figura \@ref(fig:epsilon)). És a dir,
$$
...
```{r epsilon, echo=FALSE, out.width="60%",fig.cap="L'àrea sota la corba al voltant d'$a$ és aproximadament igual a la del rectangle d'alçada f~X~(a)"}
...
curve(dnorm(x),-4,4,col="blue",lwd=3,main="",xlab="",ylab="")
abline(v=0)
```
...
```{block2,type="rmdcaution"}
Quan s'aproxima una variable discreta $X$ que només pot prendre com a valors nombres naturals, com ara una binomial o  una Poisson, per mitjà d'una variable normal $Y$, és convenient aplicar l'anomenada **correcció de continuïtat**: per a cada $n\in \mathbb{N}$, aproximar:
...
* La correcció de continuïtat ens diu que és millor aproximar $P(X\leq 70)$ per mitjà de $P(Y< 70+1/2)$:
```{r}
pnorm(70.5,80,8)
```
...
* $z_{0.95}=1.64$; és a dir, $P(Z\leq 1.64)=0.95$ i per tant $P(Z\leq -1.64)=P(Z\geq 1.64)=0.05$ (és a dir, $z_{0.05}=-1.64$) i
$$
P(-1.64\leq Z\leq 1.64)=0.9.
$$


* $z_{0.975}=1.96$; és a dir, $P(Z\leq 1.96)=0.975$ i per tant $P(Z\leq -1.96)=P(Z\geq 1.96)=0.025$ (és a dir, $z_{0.025}=-1.96$) i
$$
...
\end{array}
$$
Això serveix per deduir fórmules o resoldre problemes com el següent, i els vostres pares ho empraven per calcular probabilitats de normals (amb taules de probabilitats de la normal estàndard); ara és més còmode usar una apli.



```{example,percentalcades}
Les alçades de les nordamericanes de 20 anys  segueixen una distribució normal. S'ha estimat que un 10% d'elles fan menys de 1.55 m, i un 30.5% fan menys de 1.60 m. Per a quina alçada $h$ es té que un 95% de les nordamericanes de 18 anys fan menys de $h$ metres?
    
    
```

Diguem $X$ a la variable "Prenc una nord americana de 20 anys i mesur la seva alçada en m". Sabem que és $N(\mu,\sigma)$, però desconeixíem $\mu$ i $\sigma$. 

Ens diuen que 
$$
P(X< 1.55)=0.1,\ P(X< 1.60)=0.305
$$
i amb això volem calcular la $h$ tal que $P(X\leq h)=0.95$, és a dir, el 0.95-quantil de $X$. El que farem serà, tipificant la $X$,  traduir la informació que ens han donat a quantils d'una normal estàndard $Z$:
$$
\begin{array}{l}
...
Concloem que un 95% de les nord americanes de 20 anys fan menys de 1.74 m.
...
Direm que $X$ és una variable \red{log-normal} quan el seu logaritme neperià $\ln(X)$ és una variable normal. O, si ho preferiu, és una variable de la forma $e^Y$ amb $Y$ normal. Moltes concentracions d'enzims o anticossos tenen distribució aproximadament log-normal.
...
Així doncs, volem calcular el valor $a$ tal que $P(Y\leq a)=0.95$, que no és res més que el 0.95-quantil de $Y$. 

Com que el logaritme és injectiu, $P(Y\leq x)=P(\ln(Y)\leq \ln(x))$. Per tant el pla és trobar el 0.95-quantil de $\ln(Y)$, diguem-li $b$, i aleshores tendrem $a=e^b$. 
...
$$
\begin{array}{l}
0.1=P(Y\leq 1)=P(X\leq \ln(1))=P(X\leq 0)\\
0.75=P(Y\leq 2)=P(X\leq \ln(2))
\end{array}
$$
...
$$
\begin{array}{l}
\displaystyle 0.1=P(X\leq 0)=P\Big(\frac{X-\mu}{\sigma}\leq \frac{-\mu}{\sigma}\Big)=P\Big(Z\leq -\frac{\mu}{\sigma}\Big)\\
\displaystyle\qquad \Longrightarrow -\frac{\mu}{\sigma}=z_{0.1}=\texttt{qnorm(0.1)}=-1.2816\\
\displaystyle 0.75=P(X\leq \ln(2))=P\Big(\frac{X-\mu}{\sigma}\leq \frac{\ln(2)-\mu}{\sigma}\Big)=P\Big(Z\leq \frac{\ln(2)-\mu}{\sigma}\Big)\\
\displaystyle\qquad \Longrightarrow \frac{\ln(2)-\mu}{\sigma}=z_{0.75}=\texttt{qnorm(0.75)}=0.6745
\end{array}
$$
Obtenim per tant el sistema d'equacions
$$
\left.
\begin{array}{l}
\mu=1.2816\sigma\\
...
Per tant $P(X\leq 1.037)=0.95$. Per tant
$P(Y\leq e^{1.037})=0.95$. Com que $e^{1.037}=2.82$, concloem que  2.82 cc de digitalina són suficients per matar el 95% dels ratolins.
...
**(16)** El temps que tarda a produir-se una determinada reacció bioquímica es distribueix segons una variable normal de mitjana 17 minuts i desviació típica 3 minuts. Sense fer cap càlcul, què podem deduir d'aquesta afirmació? Marca totes les respostes correctes:

1.  Tots aquests temps se situen entre 8 i 26 minuts. 
1.  Gairebé tots aquests temps se situen entre 11 i 23 minuts. 
1.  És estrictament més probable que una reacció d'aquestes tardi entre 16 i 18 minuts que tardi entre 18 i 20 minuts. 
1.  És estrictament més probable que una reacció d'aquestes tardi entre 18 i 20 minuts que tardi entre 16 i 18 minuts. 
1.  És estrictament més probable que una reacció d'aquestes tardi entre 18 i 20 minuts que tardi entre 14 i 16 minuts.
1.  Cap de les afirmacions anteriors és correcta.


**(17)**  El temps que tarda a produir-se una determinada reacció bioquímica es distribueix segons una variable normal de mitjana 17 minuts i desviació típica 3 minuts. Quina és la probabilitat que tardi menys de 17 minuts?
...
**(18)**  El temps que tarda a produir-se una determinada reacció bioquímica es distribueix segons una variable normal de mitjana 17 minuts i desviació típica 3 minuts. Quina o quines de les afirmacions següents són vertaderes?
...
1.  En un 95% de les ocasions tarda aproximadament entre 14 i 20 minuts
1.  Tarda més de 20 minuts amb la mateixa freqüència amb la qual tarda menys de 14 minuts 
1.  Tarda més de 20 minuts amb la mateixa freqüència amb la qual tarda menys de 20 minuts
...
**(21)** Si una variable aleatòria normal té mitjana 18.1 i desviació típica 1.2, què val el seu 3er quartil (arrodonit a una xifra decimal)? (Has d'emprar R o una apli per  calcular-lo)
...
1. Que si prenem una mostra aleatòria d'individus i calculam la mitjana de les seves concentracions de creatinina, en un 95% de les ocasions aquesta mitjana estarà entre 0.66 i 1.09 mg/dl. 
