...
Amb l'estimació puntual intentam endevinar el valor d'un paràmetre d'una variable poblacional. Però hem de tenir clar que és molt difícil que amb una mostra poguem encertar exactament el valor del paràmetre. Les técniques de l'estadística inferencial ens permeten estimar aquest valor a partir d'una mostra amb un cert marge d'error, és a dir, amb una certa precisió, i una certa seguretat en el resultat. l'estimació a partir d'una sola no ens indica la precisió d'aquesta estimació. Això es fa complementant l'estimació puntual amb un interval al voltant d'aquesta estimació "on estam bastant segurs que cau el valor real del paràmetre".
...
L'amplada d'aquest interval dependrà:

* De la variabilitat de l'estimador: com menys variabilitat tingui, més precisa serà l'estimació. Normalment, la variabilitat de l'estimador creix amb la desviació típica de la variable poblacional i decreix amb la mida de les mostres.
...
L'interval que s'obté amb aquesta fórmula conté el valor del paràmetre poblacional per al Q% de les mostres aleatòries simples de mida $n$.
```

És a dir:

> Un Q% dels IC Q% contenen el valor real del paràmetre poblacional que es vol estimar.


*Tenir una confiança del Q%* significa doncs que **empram una fórmula que encerta el Q% de les vegades**, en el sentit que si aplicàssim moltíssimes vegades aquesta fórmula a mostres aleatòries simples preses a l'atzar, un Q% dels intervals que obtendríem contendrien el valor real del paràmetre que volem estimar. Però **un (100-Q)% de les vegades s'equivoca**, és a dir,  un (100-Q)% dels intervals que obtendríem **no** contendrien el valor real del paràmetre que volem estimar. *I no sabem quin és el nostre cas*.
...
Això significa que estam "*molt segurs*" que l'augment mitjà d'alcohol en sang  d'una persona després de beure 4 canyes de cervesa està entre el 40.53% i el 41.87%, perquè aquest interval l'haurem calculat amb una fórmula que el 95% de les vegades que s'aplica a una mostra aleatòria simple de mida 40 dóna un interval que conté la mitjana poblacional que volem estimar. Nosaltres som optimistes i "confiam" que el nostre interval pertany a aquest 95% d'intervals que l'encerten.
...
```{block2,type="rmdcaution"}
Sovint calculareu un interval de confiança del Q% per a un cert paràmetre $\theta$ d'una població, us donarà $[a,b]$, i amb el poc rigor amb el qual us soleu expressar, us serà igual dir 
...
La primera frase diu que $\theta$ varia i un Q% dels seus valors pertanyen a $[a,b]$. Això és fals. "El valor real de $\theta$" és un número que no varia. Per a la nostra població val una cosa concreta, desconeguda però concreta, que pertanyerà o no a l'interval $[a,b]$.
...
Encara que el millor que podeu fer és no mesclar probabilitat amb confiança, i dir simplement que teniu un 95% de confiança, o un 95% de seguretat, en què $[a,b]$ conté el valor real de $\theta$. Així no us enganxau els dits.
```
...
**Per exemple:** si un IC 95% per a la **prevalença** $p$ d'una determinada característica en una població (la fracció d'individus que tenen aquesta característica) va de 0.025 a 0.047:
...
En aquest cas, el que direm és que **no podem rebutjar** que $p$ sigui 0.03, perquè 0.03 pertany a l'interval on creiem que cau el valor real de $p$.
...
* **No paramètrics**: Tots els altres. El nostre preferit és el **bootstrap**:
    ...
    * De la mostra, es prenen a l'atzar moltes (milers) mostres aleatòries simples (és a dir, amb reposició) de la mateixa mida que la mostra.
    * Es calcula l'estimador amb cada una d'aquestes mostres.
    * S'usa el vector de resultats per estimar un interval de confiança. Per exemple, podríem prendre com a IC 95% l'interval entre els quantils 0.025 i 0.975 d'aquest vector.
...
```{block2,type="rmdnote"}
El *bootstrap* és una eina molt poderosa per calcular intervals de confiança i, en general, per estimar la distribució mostral d'un estadístic. Tant, que a la pràctica ja comença a substituir els mètodes paramètrics. Emperò no fa miracles: si la mostra és petita o molt poc representativa de la població, un IC calculat amb el bootstrap servei de tan poc com un de calculat amb un mètode paramètric.
```
...
**()** En un estudi transversal sobre una mostra de 500 subjectes representatius d'una comunitat, s'ha observat una prevalença d'una certa malaltia del 20% (IC 95%:
16.5%-23.5%). Quina de les  afirmacions següents és correcta? 


1. Si prenem una altra mostra de 500 subjectes de la mateixa comunitat, tenim un 95% de confiança que l'interval 16.5%-23.5% contendrà el percentatge de subjectes de la mostra que té aquesta malaltia.
...
1. Hi ha un 95% de probabilitats que la prevalença de la malaltia a la comunitat sigui del 20%.
1. Totes les altres respostes són falses.




**()** Prenem una mostra aleatòria simple de mida 50 d'una variable aleatòria. Calculam  un interval de confiança del 90% per a la mitjana de la variable aleatòria a partir d'aquesta mostra, dóna [11.8,12.8]. Què significa això? 
...
**()** En un estudi transversal sobre una mostra de subjectes representatius d'una comunitat, s'ha observat una prevalença d'hipertensió arterial (HTA) del 20% (interval de confiança del 95%: 15%-25%). Quina de les  afirmacions següents és vertadera?
...
**()** Un interval de confiança del 99% per a la concentració d'un determinat metabolit a la sang dels individus d'una determinada espècie és [10,12]. D'acord amb això, esperam trobar fora d'aquest interval:


1. Un 1% de les concentracions mitjanes de totes les mostres de qualsevol mida d'individus d'aquesta espècie.
1. Un 1% de les concentracions mitjanes de les mostres grans ($\geq 40$) d'individus d'aquesta espècie.
...
**()** Quina de les situacions següents ens permet una estimació més precisa d'una mitjana poblacional $\mu$ amb la fórmula basada en la t de Student?

1. $\overline{x}=10$, $\widetilde{s}=0.2$, $n=30$.
1. $\overline{x}=20$, $\widetilde{s}=0.2$, $n=30$.
1. $\overline{x}=10$, $\widetilde{s}=0.2$, $n=50$.
1. $\overline{x}=40$, $\widetilde{s}=0.1$, $n=50$. 



**()** Prenem una mostra aleatòria simple de mida 100 d'una variable aleatòria i calculam  un interval de confiança del 95% per a $\mu$ amb la fórmula basada en la t de Student. Obtenim [11.8,12.8]. Què ha valgut, aproximadament, la desviació típica mostral de la mostra?
...
**()** Per calcular un interval de confiança al 95% per al valor mitjà $\mu$ d'una certa variable, hem pres una mostra aleatòria simple de mida 100. Si ara prenguéssim una altra mostra aleatòria simple de  mida 100 i amb aquesta calculàssim un interval de confiança del 99% per a $\mu$, com seria aquest interval?
...
**()** En una mostra de 88 estudiants, es trobà que un 8% consumien begudes energètiques  (IC 95%: 2% a 14%, mètode de Laplace). Quines de les  afirmacions  següents són certes?
...
**()** Prenem una mostra aleatòria de 200 residents de Santa Margalida (població, 12000 habitants) per calcular un interval de confiança de la proporció dels margalidans que presenten una determinada condició. Quines de les afirmacions següents són vertaderes? Marcau totes les respostes vertaderes:
...
**()** Volem estimar el nivell mitjà de potassi en sang en un poble de Mallorca, mesurant-lo sobre una mostra aleatòria d'habitants del poble. L'exactitud de l'estimació pot dependre de (marcau totes les respostes correctes):
...
**()**  Prenem una mostra aleatòria simple de mida 50 d'una variable aleatòria. Calculam  un interval de confiança del 90% per a la $\mu$ de la variable aleatòria a partir d'aquesta mostra, i dóna [11.8,12.8]. Què podem deduir d'aquest fet?


1. Que el 90% dels valors de la variable poblacional estan dins l'interval  [11.8,12.8].
1. Que el 90% de les mostra aleatòria simple de mida 50 tindran la seva mitjana mostral dins l'interval [11.8,12.8].
1. Que el 90% de les mostra aleatòria simple de qualsevol mida tindran la seva mitjana mostral dins l'interval [11.8,12.8].
1. Que l'interval [11.8,12.8]  conté el vertader valor de $\mu$.
1. Que el vertader valor de $\mu$ serà, amb probabilitat 0.9, o bé 11.8 o bé 12.8.
1. Cap de les  respostes anteriors és correcta. 



**()** En una mostra aleatòria d'individus d'una comunitat s'ha obtingut una valor mitjà de la tensió arterial diastólica (TAD) de 85 mmHg, amb un error estàndard de 2.5 mmHg. Quines de les  afirmacions següents són vertaderes?
...
**()** Per calcular un interval de confiança al 95% per al valor mitjà $\mu$ d'una certa variable, hem pres una mostra aleatòria simple de mida 100. Si, amb la mateixa mostra, calculàssim un interval de confiança del 99% per a $\mu$, com seria aquest interval?
...
**()** Per calcular un interval de confiança al 95% per el valor mitjà d'una certa variable, havíem decidit prendre una mostra aleatòria simple de mida 200, però resulta massa costós, i hem decidit reduir la mida a 100. Té aquesta decisió qualque efecte sobre l'amplada de l'interval de confiança?
...
**()**  Calculam un interval de confiança al 95% per a la mitjana poblacional d'una variable aleatòria, a partir d'una mostra aleatòria simple de 100 individus.
...
**()** Amb la finalitat de disminuir el marge d'error en l'estimació de la mitjana d'una certa v.a. per mitjà de la mitjana aritmètica d'una mostra, el millor que podem fer és (marcau una única resposta):

1. Disminuir la variància de la variable aleatòria.
...
1. Com que vol calcular l'interval de confiança en el pitjor dels casos,  ja no li fa falta cap altra dada. 
