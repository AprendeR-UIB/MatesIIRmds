...
Per adquirir un poc de disciplina en la realització de contrastos d'hipòtesis, procurau acostumar-vos a dividir-los en els apartats següents:
...
> Hem obtingut evidència estadísticament significativa (test realitzat, p-valor ..., IC 95% ...) que ...  
> No hem obtingut evidència estadísticament significativa (test realitzat, p-valor ..., IC 95% ...) que ...
...
> Hem obtingut evidència estadísticament significativa  (test t, p-valor 0.03, IC 95% $-\infty$ a 456.2) que el pes mitjà actual és menor que 460 g i que per tant ha minvat en els darrers anys.
...
*Conclusió*:  Hem obtingut evidència estadísticament significativa (test t, p-valor 3·10^-8^, IC 95% 98.2 a 98.4) que la temperatura mitjana del cos humà no és de 98.6^o^ F.
...
*Conclusió*:  No hem obtingut evidència estadísticament significativa   (test t, p-valor 0.99, IC 95%  per a la diferència $-\infty$ a 0.46) que la temperatura mitjana de les dones sigui més baixa que la dels homes.
...
*Conclusió*: Hem obtingut evidència estadísticament significativa (test t, p-valor 0.003, IC 95%  per a la diferència $-\infty$ a -0.163) que desdijunar *oatbran* redueix el colesterol respecte dels *corn flakes*.
...
*Conclusió*: Hem obtingut evidència estadísticament significativa (test t, p-valor<10^-16^, IC 95% per a la diferència $-\infty$ a -0.84) que el valor mitjà del logaritme del nivell de triglicèrids a la sang del cordó umbilical d'un nadó és més petit que el valor mitjà del logaritme del nivell de triglicèrids a la sang d'un nadó de 2 setmanes 
.
...
*Conclusió*: No hem obtingut evidència estadísticament significativa (test de Wilcoxon, p-valor 0.1) que l'augment mitjà de pes en aquestes condicions sigui més petit que 70 g.
...
*Conclusió* Hem obtingut evidència estadísticament significativa (test de Wilcoxon, p-valor 6.4·10^-6^) que el nivell mitjà de triglicèrids a la sang del cordó umbilical dels nadons és més petit que a la sang dels nados de dues setmanes.
...
*Conclusió*: No hem obtingut evidència estadísticament significativa (test $\chi^2$, p-valor 0.504, IC 95% 0.202 a 0.435)
que la desviació típica (de la quantitat de proteïna total ...) sigui diferent de 0.25.
...
*Conclusió*: No hem trobat evidència estadísticament significativa (test F, p-valor 0.33) que les variables $X_h$ i $X_d$ tenguin variància diferent: acceptarem que tenen la mateixa variància.
...
*Conclusió*: No hem trobat evidència estadísticament significativa (test F, p-valor 0.33, IC 95% per al quocient 0.83 a 1.74) que les variables $X_h$ i $X_d$ tenguin variància diferent: acceptarem que tenen la mateixa variància.
...
*Conclusió*: No hem obtingut evidència estadísticament significativa (test binomial, p-valor 0.37) que el percentatge d'esquerrans a la UIB sigui diferent del 10%.
...
*Conclusió*: No hem obtingut evidència estadísticament significativa (test binomial, p-valor 0.37, IC 95% 0.001 a 0.172) que el percentatge d'esquerrans a la UIB sigui diferent del 10%.
...
*Conclusió*: Un altre cop, no obtenim evidència estadísticament significativa (prop-test, p-valor 0.222) que el percentatge d'esquerrans a la UIB sigui diferent del 10%.
...
*Conclusió*: No hem trobat evidència esatdísticament significativa (test $\chi^2$, p-valor 0.57) que les proporcions de mallorquins i menorquins amb aquest al·lel siguin diferents. 
...
*Conclusió*: No hem obtingut evidència estadísticament significativa (test de Fisher, p-valor 0.78) que la SIDS tengui un component genètic.
...
$$
\begin{array}{c}
\hphantom{Variablessss}  \text{Variable $X_2$}\\
\begin{array}{r|c|c|}
\text{Variable $X_1$} &\text{Èxit} & \text{Fracàs} \\  \hline
\text{Èxit} & a & b \\
\text{Fracàs} & c & d\\ \hline
\end{array}
\end{array}
$$
...
Z^2=\frac{(b-c)^2}{b+c}
$$
que, si se satisfan les condicions esmentades, segueix aproximadament una distribució $\chi^2_1$ si $H_0$ és vertadera.

Auest tesy està implementat en R en la funció `mcnemar.test`, que s'aplica s'aplica a la taula de freqüències absolutes de la mostra tal i com l'hem donada.
...
\end{array}
$$
on "Sí" indica que el malalt va que sí que havia notat alleujament en el dolor.

*Variables d'interès*:

* $X_1$: "Prenem un malalt, li administram el fàrmac  i li demanam si nota allejament del dolor". És Bernoulli de proporció poblacional $p_1$

* $X_2$: "Prenem un malalt, li administram el placebo i li demanam si nota allejament del dolor". És Bernoulli de proporció poblacional $p_2$
...
$$
\begin{array}{l}
2\min\big\{P(Z^2\geq 8.067),P(Z^2\leq 8.067)\big\}\\
\qquad =2\min\{\texttt{1-pchisq(8.067,1)},\texttt{pchisq(8.067,1)}\}\\
\qquad =2\min\{`r round(1-pchisq(8.067,1),3)`,`r round(pchisq(8.067,1),3)`\}=`r 2*min(c(round(1-pchisq(8.067,1),3), round(pchisq(8.067,1),3)))`
\end{array}
$$

*Conclusió*: Hem obtingut evidència estadísticament significativa (test de McNemar, p-valor 0.007) que la taxa d'efectivitat del placebo i del fàrmac són diferents.  
...
\begin{array}{r|c|c|}
\text{Variable $X_1$} &\text{Èxit} & \text{Fracàs} \\  \hline
\text{Èxit} & p_{11} & p_{01} \\
\text{Fracàs} & p_{10} & p_{00}\\ \hline
\end{array}
\end{array}
$$
...
El podem efectuar amb un test binomial exacte prenent:
...
L'inconvenient d'aquest test és l'interval de confiança que donarà és per a $p_{10}/(p_{10}+p_{01})$, i no té res a veure amb $p_1-p_2$ o $p_1/p_2$. Per tant només hi heu de mirar el p-valor.
...
*Conclusió*: Hem obtingut evidència estadísticament significativa (test binomial, p-valor 0.003) que la taxa d'efectivitat del placebo i del fàrmac són diferents.  
