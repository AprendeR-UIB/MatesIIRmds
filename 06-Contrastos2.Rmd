...
```{block2,type="rmdnote"}
Un test t de mostres aparellades és en realitat un test t de la mitjana de $X_1-X_2$. Per tant, si la mida $n$ de les mostres no és gran, no  cal que les dues variables poblacionals $X_1$ i $X_2$ siguin totes dues normals per poder-lo fer servir. Basta que la seva diferència $X_1-X_2$ sigui normal.  
```
...
És a dir, el contrast que resoldrem serà
$$
\left\{\begin{array}{l}
H_0:\ \text{$X_{cu}$ i $X_{ds}$ tenen la mateixa distribució}\\
H_1:\ \text{$\mu_{cu}<\mu_{ds}$}
\end{array}\right.
$$
...
1. Concatenam en un únic vector les mostres $A$ i $B$. Obtenim un vector de $n_A+n_B$ dades, les primeres $n_A$ d'una mostra i les altres $n_B$ de l'altra mostra.
...
2. Repetim 5000 vegades el procés següent:
     ...
     a. Prenem una m.a.s. de `Global` de la seva mateixa mida, $n_A+n_B$
     a. Calculam la mitjana de les primeres $n_A$ entrades
     a. Calculam la mitjana de les darreres  $n_B$ entrades
     a. Calculam la diferència d'aquestes mitjanes
...
Remostreig=replicate(5000,DifM(sample(Global, nA+nB, rep=TRUE),nA,nB))
```

3. Prendrem com a p-valor la proporció d'aquestes diferències de mitjanes que són més petites o iguals que $\overline{A}-\overline{B}$.
...
on `x` indica al nombre d'èxits a la mostra, `n` la mida de la mostra, `p` la probabilitat que es contrasta, i els altres dos paràmetres signifiquen el mateix que a les altres funcions explicades fins ara i els seus valors per defecte són els de sempre (`"two.sided"` i `0.95`, respectivament). L'interval de confiança que dóna en els contrastos bilaterals és el de Clopper-Pearson. 
...
\qquad = P(S\leqslant 1)+P(S\geqslant 5)\\
...
i a banda del p-valor obtenim l'IC 95% de Clopper-Pearson per a $p$
...
```{block2,type="rmdnote"}
Aquest test s'anomena **prop-test**, o també **test** $\chi^2$ perquè és un cas particular d'un test $\chi^2$ que veurem al Tema \@ref(chap:bondat). De fet, com en el cas d'una proporció, en la seva implementació en R no s'hi empra $Z$ sinó $Z^2$, que té distribució aproximadament $\chi^2_1$ si $p_1=p_2$.
```
...
*Valor de l'estadístic*: Tenim que $\widehat{p}_1=0.2$, $\widehat{p}_2=0.24$, $n_1=100$, $n_2=50$, $E=32$ i per tant
$$
z_0=\frac{0.2 -0.24}{
...
Z^2=\frac{(b-c)^2}{b+c}
$$
el qual, si se satisfan les condicions esmentades, segueix aproximadament una distribució $\chi^2_1$ si *H*~0~  és vertadera. 
...
1. Si vius en una comunitat urbana dispersa, tens un 20% manco de probabilitat de sofrir la COVID-19 que si vius en una ciutat
