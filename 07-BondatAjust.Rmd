...
```{example,grossa}
Les freqüències de les darreres xifres dels primers premis de la Grossa de la loteria de Nadal als 212  sortejos de la seva història (fins el de 2020, inclòs) han estat les següents:
...
\text{freqüència} &  22 & 8 & 13 & 21 & 27 & 31 & 27 & 23 & 23 & 17\\
\hline
\end{array}
$$
...
Per exemple, continuant amb el nostre exemple de la Grossa, si totes les darreres xifres tinguessin la mateixa probabilitat d'aparèixer, la seva probabilitat seria 1/10, i per tant el valor esperat d'aparicions de cadascuna d'elles seria 212/10=21.2.
...
* $esp_i$: la freqüència absoluta **esperada** d'aquesta classe  si $H_0$ és vertadera: $esp_i=p_i\cdot n$. En el nostre exemple de la loteria, totes aquestes freqüències absolutes $esp_i$ valen el mateix, 212/10=21.2.
...
\end{array}
$$
Com que totes les freqüències esperades valen 21.2, el valor de l'estadístic de contrast sobre aquesta mostra és
$$
\begin{array}{rl}
 \chi_0^2& \displaystyle=\frac{(22-21.2)^2}{21.2}+\frac{(8-21.2)^2}{21.2}+\cdots\\[2ex]
 & \qquad\quad\displaystyle +\frac{(23-21.2)^2}{21.2} +\frac{(17-21.2)^2}{21.2}=20.26
\end{array}
$$
...
*Valor de l'estadístic de contrast*: Ja l'hem calculat, 20.26.

*p-valor*: $P(\chi_9^2\geqslant 20.26)=\texttt{1-pchisq(20.26,9)}= 0.0164$ 
...
```{example,grossasim}
Anem a confirmar amb una simulació aquest p-valor. El que farem serà el següent: Repetirem 10000 vegades, amb un `replicate`, el procés d'extreure a l'atzar i de manera equiprobable 212 dígits, calcular-ne la taula de freqüències de cada una d'aquestes mostres i calcular l'estadístic de contrast $\chi^2$ sobre cada una d'aquestes taules. Finalment, calcularem la fracció de vegades que $\chi^2$ ha valgut més que a la nostra mostra, on ha donat 20.26.
...
```{block2,type="rmdimportant"}
L'estimació dels paràmetres s'ha de realitzar amb els estimadors màxim versemblants, que són els que maximitzen la probabilitat de la nostra mostra. D'aquesta manera, si amb els paràmetres que fan màxima la nostra mostra, aquesta ens surt molt rara, més evidència tenim que el tipus de distribució no és el correcte.
```
...
\left\{
\begin{array}{l}
H_0: \text{ aquesta mostra prové d'una v.a. $X$ $N(3,1.5)$}\\
H_0: \text{ aquesta mostra no prové d'una v.a. $X$ $N(3,1.5)$}
\end{array}
\right.
$$
...
3. Comparam $F_n(x)$ amb la funció de distribució $F_X(x)$ que tendria la variable aleatòria $X$ si satisfés la hipòtesi nul·la. Si són molt diferents, rebutjarem que la mostra prové d'una variable amb aquesta funció de distribució $F_X$. 
...
**(11)** Va, vegem si llegiu els apunts. Si $X$ és variable$\chi_n^2$, quin dels valors següents és el més petit d'aquesta llista al que **us dic** en una nota que la probabilitat que $X$ sigui més gran que ell és molt petita?
...
**(13)** Podem emprar un test de Kolgomorov-Smirnov per contrastar a partir  d'una mostra si una variable poblacional és normal, per a alguns valors dels paràmetres $\mu$ i $\sigma$?
