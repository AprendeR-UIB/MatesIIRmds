# Contrastos d'independència i homogeneïtat (Provisional)

En els contrastos d'independència de dues variables $X$ i $Y$, la hipòtesi nul·la és que no hi ha cap relació entre $X$ i $Y$, és a dir, que són independents. Quan traduïm la independència de dues variables en termes de "la probabilitat de la intersecció és el producte de probabilitats", resulta que un contrast d'independència és un tipus concret de contrast de bondat d'ajust.

En els contrastos d'homogeneïtat  de dues variables $X$ i $Y$, la hipòtesi nul·la és que la distribució de $Y$ condicionada  a cada un dels valors que pot prendre $X$ és sempre la mateixa, que és una altra manera de dir que $X$ i $Y$ són independents. Per tant, els contrastos d'homogeneïtat són formalment contrastos d'independència, però difereixen en la forma com s'hi recull la mostra:
...
1. Mesuram les variables aleatòries sobre una mostra aleatòria simple de $n$ subjectes, i obtenim una taula de contingència de freqüències absolutes com la següent (on $n_{i,j}$ indica nombre de subjectes amb $X=X_i$ i $Y=Y_j$; $n_{i\bullet}$  indica nombre de subjectes amb $X=X_i$; i $n_{\bullet j}$  indica nombre de subjectes amb $Y=Y_j$):
...
n_{sj} &
\ldots & n_{st} & n_{s \bullet} \\ \hline text{Total} & n_{\bullet
1} &
...
```{theorem}
Si les variables $X$ i $Y$ són independents,  $n$ és gran (diguem $n\geq 30$) i cada freqüència esperada 
$$
esp_{ij}=\frac{n_{i\bullet}\cdot n_{\bullet j}}{n}
$$
és $\geq 5$,  l'estadístic
$$
\chi^2=\sum\limits_{i=1}^s\sum\limits_{j=1}^t \frac{ ( n_{ij}- esp_{ij})^2 } {esp_{ij}}
$$
...
el nombre de graus de llibertat és el nombre de classes, $st$ (una per cada combinació de possible valor de $X$ i possible valor de $Y$) menys 1 i menys el nombre de paràmetres estimats:
$$
...
$$
\text{p-valor}=P(\chi_{(s-1) \cdot (t -1)}^2\geq \chi_0^2)
$$
...
La taula següent mostra la classificació d'una mostra de  444 nens segons el seu CCV  i el nivell sociocultural dels pares:
$$
\begin{array}{l}
\hphantom{Compli CCV BaixBB}\text{Nivell sociocultural}\\
\begin{array}{r|ccc|}
\text{Compliment CCV} &\text{Baix}& \text{Mitjà} &\text{Alt}\\ \hline
\text{Baix} &38 &76 &79\\
...
R ens avisa que no es compleixen les condicions per usar el test $\chi^2$: segurament hi ha freqüències esperades petites. Vegem-ho, i aprofitarem per fer-ho amb R:
...
* Les $n_{i\bulletj}$ són les sumes de les columnes:
```{r}
Freq.NSC=colSums(TaulaCCV)
Freq.NSC
```

* La matriu de les $(n_{i\bullet}n_{\bullet j}/n)$ s'obté fent el producte matricial
$$
\begin{array}{l}
...
```{r}
Freqs.Esp=(1/sum(TaulaCCV))*matrix(Freq.CCV,ncol=1,byrow=TRUE)%*%matrix(Freq.NSC,nrow=1,byrow=TRUE)
Freqs.Esp
```
...
```{block2,type="rmdnote"}
Per que fos un contrast d'independència, hauríem d'haver pres 300 malalts d'hipercolesterolèmia que estiguessin prenent algun d'aquests tractaments, i hauríem anotat de cada un quin tractament pren i si ha tengut èxit. Amb aquesta mostra transversal, hi havia el risc que algun tractament fos seguit per molt pocs, o fins i tot cap, malalt.
```
...
De la mateixa manera que les dues variables involucrades en un contrast d'independència podien tenir més de dos nivells, podem efectuar contrastos d'homogeneïtat en situacions més generals que la comparació de variables Bernoulli.
...
* *Y*: "Prenem un pacient i miram si al cap de 5 setmanes en quina classe està el seu nivell de colesterol"
...
*Conclusió*: No hem trobat evidència estadísticament significativa que A, B i C no tenguin el mateix efecte quan distingim tres classes de nivell de colesterol (test $\chi^2$ d'homogeneïtat, p-valor 0.09). La conclusió concreta ja dependria de si prenguéssim nivell de significació 0.05 o o.1.
...
\begin{array}{ll}
H_0: \text{ $p_1=\cdots=p_k$}\\
H_1: \text{ $p_1\leq\cdots \leq p_k$ i almenys una }\\
\hphantom{H_1: } \text{ d'aquestes desigualtats és estricta}
\end{array} 
\right.
$$

Fixau-vos que "$p_1\leq\cdots \leq p_k$ i almenys una  d'aquestes desigualtats és estricta" és equivalent a "$p_1\leq\cdots \leq p_k$ i $p_1<p_k$", i aquesta darrera reformulació és més curta.


Es duu a terme amb un **test $\chi^2$ de Cochran-Armitage**, una variant del test $\chi^2$ explicat fins ara.
Per utilitzar-lo, basta que la mostra total sigui gran ($\geq 30$), no cal la condició de Cochran.
...
\begin{array}{ll}
H_0: \text{ $p_N=p_O=p_R$}\\
H_1: \text{ $p_N\leq p_O  \leq p_R$ i $p_N<p_R$}
\end{array} 
\right.
$$
