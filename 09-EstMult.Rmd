...
```{r, echo=FALSE, fig.asp = 0.33,  fig.width=15,out.width="90%",fig.cap="Significat del signe de la covariància"}
par(mfrow=c(1,3))
set.seed(100)
x=rnorm(50,5,1)
y=x+rnorm(50,0,0.5)
...
set.seed(300)
t=rnorm(50,6,2)
plot(x,y,pch=20,xlab="",ylab="",main="Covariància >0",cex=2)
plot(x,z,pch=20,xlab="",ylab="",main="Covariància <0",cex=2)
plot(x,t,pch=20,xlab="",ylab="",main="Covariància =0",cex=2)
par(mfrow=c(1,1))
```
...
```{r, echo=FALSE, out.width="80%",fig.cap="Correlació no implica causalitat, segons \"Spurious Correlations\""}
...
$$
\rho_{X_1,X_2-X_1}=-\frac{1}{\sqrt{2}}\approx 0.7
$$
```
...
```{block2,type="rmdimportant"}
Per tant, com més s'acosta la correlació de Pearson de $x$ i $y$, en valor absolut, a 1, més s'acosten els vectors $x,y$ a dependre linealment l'un de l'altre, és a dir, més s'acosten els punts $(x_i,y_i)$ a estar sobre una recta. El signe de $R_{X,Y}$ indica si aquesta dependència és creixent ($R_{X,Y}>0$) o decreixent ($R_{X,Y}<0$).
```
...
1. Qualsevol valor real, i també $\pm\infty$
...
1. Valen el mateix si els vectors són mostres de variables ordinals
1. Valen el mateix si els valors dels dos vectors són nombres enters
1. La correlació de Spearman no es pot emprar amb mostres de variables quantitatives
1. Si la correlació de Pearson de dos vectors és 1, la de Spearman també
1. Si la correlació de Spearman de dos vectors és 0, la de Pearson també
