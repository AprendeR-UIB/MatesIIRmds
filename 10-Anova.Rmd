...
Aquest experiment és un cas particular de la situació següent:
...
Si la variabilitat total de les dades és molt més gran que la suma de les variabilitats dins les mostres, hi haurà una gran variabilitat de les mitjanes mostrals i serà senyal que les mitjanes poblacionals no són iguals.
```

Per exemple, si tenim molta variabilitat global
...
Si en canvi tenim molta variabilitat global
...
```{block2,type="rmdrecordau"}
**No basten mostres grans**. Aquí no hi juga cap paper el Teorema Central del Límit. Però, per sort, les conclusions d'una ANOVA són bastant robustes encara que falli la condició de normalitat de les subpoblacions. En canvi són sensibles a l'**heterocedasticitat** (el contrari d'homocedasticitat; "hetero" indica el contrari de "homo").
```
...
* $\overline{X}_{i}$: La mitjana mostral de la mostra del tractament $i$-èsim: 
$$
\overline{X}_{i} =
...
* $\overline{X}$: La mitjana mostral de tota la mostra: 
$$
...
$$
hem d'observar que les mitjanes $\overline{X}_{i}$ formen la variable **Inc** del *dataframe* `mitjanes.tracts`

```{r}
mitjanes.tracts$Inc
```
i els $n_i$ són les freqüències absolutes de cada nivell dins la variable `CO2$Pre`
```{r}
table(CO2$Pre)
```
Per tant la fórmula que defineix $SS_{Tr}$ se tradueix en
```{r}
...
Si no estau còmodes amb això de restar vectors de longituds una un múltiple de l'altra, el que heu de fer és restar a `CO2$Inc` un vector format per 10 còpies consecutives de `mitjanes.tracts$Inc`:
```{r}
...
```{block2,type="rmdrecordau"}
En particular, si se satisfan les condicions necessàries per poder fer una ANOVA, **$MS_E$ és un estimador no esbiaixat de  la variància poblacional comuna  de tots els tractaments, $\sigma^2$**.
```
...
$$
que surt tal qual al denominador de $T^2$. Per tant ens queda veure que $MS_{Tr}$ és igual a
$$
...
&\quad \displaystyle  = n_1\overline{X}_1^2-2n_1\overline{X}_1\overline{X}+n_1\overline{X}^2+ n_2\overline{X}_2^2-2n_2\overline{X}_2\overline{X}+n_2\overline{X}^2\\
&\quad \displaystyle  = S_1\overline{X}_1+S_2\overline{X}_2-2S_1\overline{X}-2S_2\overline{X}+(n_1+n_2)\overline{X}^2\\
&\quad \displaystyle  =S_1\overline{X}_1+S_2\overline{X}_2-2(S_1+S_2)\overline{X}+(S_1+S_2)\overline{X}\\
&\quad \displaystyle  =S_1\overline{X}_1+S_2\overline{X}_2-(S_1+S_2)\overline{X}\\
&\quad \displaystyle  = S_1\overline{X}_1+S_2\overline{X}_2 -\frac{(S_1+S_2)(n_1\overline{X}_1+n_2\overline{X}_2)}{n_1+n_2}\\
&\quad \displaystyle  =\frac{(n_1+n_2)S_1\overline{X}_1+(n_1+n_2)S_2\overline{X}_2-n_1S_1\overline{X}_1-n_1S_2\overline{X}_1-n_2S_1\overline{X}_2-n_2S_2\overline{X}_2}{n_1+n_2}\\
&\quad \displaystyle  =\frac{n_2S_1\overline{X}_1+n_1S_2\overline{X}_2-n_1S_2\overline{X}_1-n_2S_1\overline{X}_2}{n_1+n_2}\\
&\quad \displaystyle  =\frac{n_2n_1\overline{X}_1^2+n_1n_2\overline{X}_2^2-n_1n_2\overline{X}_2\overline{X}_1-n_2n_1\overline{X}_1\overline{X}_2}{n_1+n_2}\\
&\quad \displaystyle  =\frac{n_1n_2(\overline{X}_1^2+\overline{X}_2^2-2\overline{X}_1\overline{X}_2)}{n_1+n_2}\\
&\quad \displaystyle  =\frac{n_1n_2(\overline{X}_1-\overline{X}_2^2}{n_1+n_2}\\
&\quad \displaystyle  =\frac{(\overline{X}_1-\overline{X}_2)^2}{\frac{n_1+n_2}{n_1n_2}}\\
&\quad \displaystyle  =\frac{(\overline{X}_1-\overline{X}_2)^2}{\frac{1}{n_1}+\frac{1}{n_2}}
\end{array}
$$
```
...
* El p-valor  $P(F_{k-1,N-k}\geqslant F_0)$ val
```{r}
1-pf(F0,k-1,N-k)
```


*Conclusió*: Hem trobat evidència estadística que el nivell de pressió de CO~2~ influeix en el creixement mitjà del microorganisme *Pseudomonas fragi* (ANOVA, p-valor `r 1-pf(F0,k-1,N-k)`).
...
summary(aov( ))
```
a la fórmula que separa les dades numèriques segons els nivells del factor (**ha de ser un factor** o, de la versió 4.0 de R endavant, **un vector de paraules**). Si la fórmula només especifica els noms de les variables, s'hi ha d'indicar el nom del *dataframe* amb el paràmetre `data`. 
...
Per exemple, a l'Exemple \@ref(exm:CO2), si realitzam els $c=\binom{5}{2}=10$ contrastos (totes les pressions contra totes les pressions) amb nivell de significació $\alpha =0.05$, la probabilitat de cometre almenys un Error de Tipus I a qualcun (si totes les mitjanes fossin iguals en la realitat) és  $1-(1-0.05)^{10} \approx 0.4$.
...
```{block2,type="rmdcaution"}
Bé, gairebé equivalents. Si quan multiplicau un dels p-valors per  $c$ us dóna un valor més gran que 1, heu de donar com a p-valor ajustat 1. Pensau que, ajustats o no, els p-valors representen probabilitats, i per tant és incorrecte donar-ne de més grans que 1.
```
...
El resultat d'un  `pairwise.t.test` dóna el triangle inferior de la matriu dels p-valors: l'entrada $(i,j)$ correspon al contrast de mitjanes del nivell $i$-èsim i el nivell $j$-èsim. 
...
1. La taula amb els p-valors dels contrastos és:
$$
\begin{array}{c|c}
...
3. Ajustam, multiplicant, per a cada $j$, el $j$-èsim p-valor (en ordre decreixent) per $6+1-j$:
...
*Conclusió*: Hem trobat evidència estadística que cada una de les diferents pressions de CO~2~ considerades dóna lloc a un increment mitjà diferent del volum cel·lular de *P. fragi* al cap d'una hora (ANOVA d'1 via, test posterior de Holm, p-valors ajustats < 0.0004).
...
```{block2,type="rmdimportant"}
Si efectuam $k$ contrastos de normalitat amb nivell de significació $\alpha$ i a tots ells la hipòtesi nul·la és vertadera, tenim una probabilitat de cometre algun error de tipus I de $1-(1-\alpha)^k$. Per tant, com als contrastos posteriors per parelles, convé ajustar els p-valors a fi que el nivell de significació global sigui $\alpha$. 
```
...
Acceptaríem (si el test de Fligner fos l'adient) que se satisfà l'homocedasticitat.
...
Tornam a concloure que els percentatges mitjans poblacionals d'increment són tots diferents.
...
```{example, esports}
Volem determinar si l'energia que  consumeixen tres activitats físiques (córrer, caminar i muntar amb bicicleta) és la mateixa o no. Quantificam aquesta energia amb el nombre de Kcal consumides per Km recorregut.
...
seleccionam l’ordre de les activitats de cada individu a l’atzar.
...
Aquests són els resultats de 8 individus:
...
En aquest estudi, fixau-vos que empram els nivells d'un factor (tipus de desplaçament) per classificar la població (els consums energètics) en tres subpoblacions, però ara les mostres que hem pres no són independents sinó **aparellades**: hem mesurat els consums energètics dels tres esports sobre els mateixos individus. Per tant no seria correcte realitzar una ANOVA d'1 via per analitzar els resultats. 
...
1. Escollim una mostra aleatòria de $b$ **blocs**: conjunts de $k$ subjectes aparellats  (per exemple, escollim $b$ subjectes i entenem cada bloc com a format per $k$ còpies del mateix subjecte), on $k$ és el nombre de tractaments.
...
on cada $X_{ij}$ és el valor del tractament $i$-èsim en l'individu corresponent del bloc $j$-èsim. Com en les taules de dades emprades en l'ANOVA d'1 via, observau que no és la notació usual, sinó la transposada: a $X_{{i}{j}}$, $i$ hi indica la **columna**,  el tractament, i $j$ hi indica la **filera**, és a dir, el bloc. D'aquesta manera, cada filera $(X_{1j},\ldots,X_{kj})$ representa un bloc.
...
* No hi hagi **interacció** entre els blocs i els tractaments. Això significa que l'efecte dels blocs i els tractaments ha de ser **additiu**: per a cada parella concreta de tractaments, la diferència entre les seves mitjanes poblacionals és la mateixa a cada bloc
$$
...
$$
    ...
    * La diferència entre el consum energètic mitjà en córrer 1 km i en caminar 1 km és la mateixa a tots els individus  de la mostra
    * La diferència entre el consum energètic mitjà en córrer 1 km i en pedalar 1 km és la mateixa a tots els individus  de la mostra
    * La diferència entre el consum energètic mitjà en caminar 1 km i en pedalar 1 km és la mateixa a tots els individus  de la mostra
...
$$
Estima la mitjana poblacional del tractament $i$-èsim, $\mu_i$.
...
$$
Estima la mitjana poblacional del bloc $j$-èsim, $\mu_{\bullet j}$.
...
```{example}
Tornem a l'Exemple \@ref(exm:esports). Recordem les dades que ja tenim:
...
b=8  #Nombre de blocs
mitjanes.tracts  #Mitjanes de tractaments
mitjanes.blocs  #Mitjanes de blocs
mitjana.total  # Mitjana global
```
...
$$
Recordau que el vector $(\overline{X}_{1},\overline{X}_{2},\overline{X}_{3})$ és la variable **kilocal** de `mitjanes.tracts`.
...
$$
Recordau que el vector $(\overline{X}_{\bullet 1},\ldots,\overline{X}_{\bullet b})$ és la variable **kilocal** de `mitjanes.blocs`.
...
$$
Per calcular aquesta suma, procedirem com al càlcul de la $SS_E$ de l'ANOVA d'1 via, tenint en compte que
    ...
    * Cada grup de 3 entrades consecutives de `Dades$kilocal` corresponen a un bloc: les tres primeres al bloc 1, les tres següents al bloc 2 etc. Per tant, per restar a cada entrada de `Dades$kilocal` la mitjana del bloc corresponent, li hem de restar `rep(mitjanes.blocs$kilocal,each=3)`.
...
* A més, R calcula el **Quadrat mitjà dels blocs**, que nosaltres no farem servir però que definim perquè sapigueu què és:
$$
...
\end{array}
$$
on $\sigma^2$ indica la variància comuna de totes les poblacions definides per les parelles (tractament,bloc). Per tant, $MS_E$ torna a ser un estimador no esbiaixat d'aquesta $\sigma^2$.
...
Obtenim la taula d'abans (llevat d'una petita diferència en la F deguda a que als càlculs a mà hem arrodonit pel camí)  amb dues entrades més: l'estadístic de contrast i el p-valor de la filera `blocs`, que corresponen al contrast sobre hi ha diferència o no entre les mitjanes dels blocs (en aquest exemple, els consums mitjans energètics per km dels individus de la mostra en desplaçar-se de qualsevol de les tres maneres).
...
```{example}
A l'experiment de l'Exemple \@ref(exm:esports) hem arribat a la conclusió que no totes les mitjanes són iguals. Emprem un contrast posterior per parelles amb l'ajust de Bonferroni per esbrinar quines són diferents.
...
*Conclusió*: Hem obtingut evidència estadística que les tres activitats tenen  consums energètics mitjans per km diferents (ANOVA de blocs, test posterior de Bonferroni, p-valors ajustats inferiors a 0.0011)
...
* $(\alpha\beta)_{ij}=\mu_{ij}-\mu_{i\bullet}-\mu_{\bullet j}+\mu$ representa l'**efecte de la interacció** entre el  nivell $i$-èsim del factor $A$ i el  nivell  $j$-èsim del factor $B$; si no hi hagués interacció, tendríem que
$$
...
```{theorem,SS2vies, name="Identitats de les sumes de quadrats"}
$$
\begin{array}{l}
...
```{block2,type="rmdexercici"}
Procurau entendre com calculam les sumes de quadrats; hem donat els raonaments detallats als altres dos tipus d'ANOVA, aquí ja no els repetirem.
```
...
```{example}
Al nostre Exemple \@ref(exm:gupis), hem obtingut les sumes de quadrats següents:
...
\end{array}
$$
    *Conclusió*: Hem trobat evidència estadística que el GSI mitjà és diferent segons el foto-període (ANOVA de 2 vies, p-valor 0.0003)
...
\end{array}
$$
    *Conclusió*: Hem trobat evidència estadística que el  GSI mitjà és diferent segons la temperatura (ANOVA de 2 vies, p-valor 2·10^-5^)
...
\end{array}
$$
    *Conclusió*: Hem trobat evidència estadística que no tots els GSI mitjans per a les combinacions de foto-període i temperatura són iguals (ANOVA de 2 vies, p-valor 9·10^-6^)
...
\end{array}
$$
    *Conclusió*: No hem trobat evidència estadística  que hi hagi interacció entre el foto-període i la temperatura (ANOVA de 2 vies, p-valor 0.051)
...
Així, l'ANOVA de 2 vies de l'Exemple \@ref(exm:esports) es fa amb
...
1. Concloure que hi ha una parella de mitjanes mostrals que són iguals quan en realitat són totes diferents
1. Concloure que totes les mitjanes mostrals són iguals quan en realitat hi ha alguna parella que són diferents
...
1. Concloure que totes les mitjanes poblacionals són iguals quan en realitat hi ha alguna parella que són diferents
...
**(10)**  Hem realitzat una ANOVA d'1 via. Les dues primeres columnes de la taula resultant són:
$$
...
1. La distribució de l'estadístic de contrast és F de Fisher amb 3 i 21 graus de llibertat 
1. La distribució de l'estadístic de contrast és F de Fisher amb 21 i 3 graus de llibertat 
...
1. 3 tests t per a mostres independents, un per a cada parella de tractaments
1. Una anàlisi de la variància 
1. Un contrast d'homogeneïtat
1. Un test de Kruskal-Wallis 
1. 3 proves de Mann-Whitney, una per a cada parella de tractaments
...
1. La suma de quadrats dels errors hagués estat més petita 
...
1. Si l'aplicam a 2 mostres, és equivalent al test de Wilcoxon 
1. Si l'aplicam a 2 mostres, és equivalent al test de Mann-Whitney 
1. Requereix que les mostres provenguin de distribucions normals.
1. Requereix que alguna de les mostres provengui d'una distribució NO normal.
...
**(20)**  Hem realitzat una ANOVA de blocs. Les dues primeres columnes de la taula resultant són:
$$
\begin{array}{|@{}l@{}|l@{}|l@{}|l@{}|l|l|}
\hline
\text{Font de}&\text{Graus de}\hphantom{x} &\text{Suma de}\hphantom{x}&\text{Quadrats}\hphantom{x} &\text{Estadístic}\hphantom{x} & \text{p-valor}\\
...
1. Que hi ha qualque parella de mitjanes mostrals dels tractaments que han estat diferents.
1. Que hi ha qualque parella de mitjanes  poblacionals dels tractaments que són diferents. 
1. Que hi ha qualque parella de mitjanes  poblacionals dels blocs que són diferents. 
1. Que hi ha qualque parella de mitjanes  poblacionals dels blocs que han estat diferents. 
