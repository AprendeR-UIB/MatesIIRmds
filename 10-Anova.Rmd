...
Recordau que el contrari de "totes les mitjanes són iguals" és **hi ha almenys dues mitjanes que són diferents**.
```
...
Usualment, aquestes $k$ poblacions seran subpoblacions d'una única població, definides pels nivells d'un o diversos factors. Per exemple, a l'estudi del creixement del *P. fragi*, les poblacions que consideram estan totes formades per cultius d'aquest bacteri, i es diferencien en la pressió de CO~2~.
...
per referir-nos a la mitjana de la població definida pel tractament "0.29 atmosferes", és a dir, al percentatge mitjà d'increment del volum cel·lular  en una hora d'un cultiu de *P. fragi* a 0.29 atmosferes de pressió de CO~2~. I parlarem de
...
Si diem $\mu_1,\ldots,\mu_k$ a les mitjanes d'aquesta variable en aquestes $k$ poblacions, la pregunta que ens plantejam correspon al contrast:
$$
\left\{
\begin{array}{l}
...
* $X_\ell$: "Prenc un cultiu de *P. fragi* a $\ell$ atmosferes de CO~2~ i mesur el percentatge de creixement del seu volum cel·lular en una hora"
...
* Cada $X_\ell$ de l'Exemple \@ref(exm:C02seq) és la restricció de $X$ a la subpoblació definida pel nivell $\ell$: els cultius realitzats a $\ell$ atmosferes de CO~2~
...
```{block2,type="rmdimportant"}
De cara a fer-ho amb R, és important que tengueu present que la variable que defineix les subpoblacions, en aquest cas la que dóna el nivell de pressió de CO~2~, ha de ser un factor o, almenys, una vector de paraules. No pot ser un vector numéric.
```
...
I ara un que mostri la variabilitat de les dades i les mitjanes com els del final de la secció anterior. Aquest gràfic normalment no el dibuixarem, simplement és per comparar-lo amb aquells gràfics. De totes formes, mostram el codi per si qualque dia voleu produir un gràfic d'aquest estil.
...
y=rnorm(10,1,0.5)
z=rnorm(10,-1,0.5)
plot(rep(3,10),z,pch=17,xlim=c(0,4),ylim=range(c(x,y,z)),col="blue",xlab="",ylab="",xaxp=c(1,3,2))
points(rep(2,10),y,pch=18,col="red")
points(rep(1,10),x,pch=19)
#
curve(dnorm(x,0,0.5),xlim=c(-3,3),lwd=2,xlab="",ylab="")
curve(dnorm(x,1,0.5),col="red",add=TRUE,lwd=2)
curve(dnorm(x,-1,0.5),col="blue",add=TRUE,lwd=2)
legend("topleft",cex=0.75,lty=c(1,1,1),col=c("black","red","blue"),legend=c(expression(X[1]),expression(X[2]),expression(X[3])))
```
...
points(rep(1,10),x,pch=19)
#
curve(dnorm(x,0,1),xlim=c(-4,4),lwd=2,xlab="",ylab="")
...
* $X_i$ representa el valor de la variable $X$ sobre un individu del nivell $i$-èssim 

* $\mu_i-\mu$ representa la desviació de la mitjana de $X$ en el nivell $i$-èssim respecte de la mitjana global $\mu$; en direm l'**efecte del tractament $i$-èssim** i representa la contribució al valor de $X$ sobre un individu del nivell $i$-èssim del fet de pertànyer a aquest nivell 

* $X_i-\mu_i$, la desviació  de la variable $X$ sobre un individu del nivell $i$-èssim respecte de la mitjana d'aquest nivell, $\mu_i$, representa el component aleatori en una medició concreta de $X$ sobre un individu del nivell $i$-èssim, és a dir, "la part de la diferència entre  $X_{i}$ i  $\mu$ que no explica el tractament"; en diem  l'**error aleatori**, o el **residu**.

Fixau-vos que la fórmula que descriu aquest model és sempre veritat (podeu simplificar les $\mu_i$ i les $\mu$ de la dreta), i bàsicament diu que  el valor de $X$ sobre un individu és la suma de tres components:

* La mitjana global 
* L'efecte del tractament
* L'error aleatori

i en particular que res més no influeix en el valor de $X$ sobre un individu. Per tant, si això no és veritat, és a dir, si aquest valor pot dependre d'altres efectes que no tinguem en compte, el model no és vàlid, i les conclusions de l'ANOVA tampoc.

Passem ara a la mostra amb la que volem realitzar el contrast. Siguin:

* $\overline{X}_{i}$: Mitjana mostral de la mostra del tractament $i$-èssim: 
$$
\overline{X}_{i} =
\frac{\sum_{j=1}^{n_i} X_{ij}}{n_i}
$$
Estima la mitjana $\mu_i$ de la subpoblació definida pel tractament $i$-èssim
...
* Si $H_0$ és certa, $E(MS_E)=E(MS_{Tr})$ i per tant hauríem d'esperar que aquests dos estadístics  prenguessin valors propers, és a dir, hauríem d'esperar que
$$
...
* Calculam el valor de l'estadístic de contrast
$$
...
El resultat d'un  `pairwise.t.test` dóna el triangle inferior de la matriu dels p-valors: l'entrada $(i,j)$ correspon al contrast de mitjanes del nivell $i$-èssim i el nivell $j$-èssim. 
...
La primera entrada d'aquest resultat, 0.06554, és el p-valor sense ajustar del contrast de $\mu_2$ contra $\mu_1$; el 0.00756 és el p-valor sense ajustar del contrast de $\mu_3$ contra $\mu_1$; etc.
...
2. Ordenam aquests p-valors en ordre creixent $p_{(1)}< \cdots< p_{(c)}$ i re-enumeram consistentment els contrastos $C_{(1)},\ldots, C_{(c)}$
...
3. Ajustam: per a cada $j$, multiplicam el $j$-èssim p-valor (en ordre decreixent) per $6+1-j$
...
*Conclusió*: Hem trobat evidència estadística que cada una de les diferents pressions de CO~2~ considerades dona lloc a un increment mitjà diferent del volum cel·lular de *P. fragi* al cap d'una hora (ANOVA d'1 via, test posterior de Holm, p-valors ajustats < 0.0004).
...
Podem acceptar que totes les mostres provenen de variables normals.
...
El resultat és un data frame, la variable `Inc` del qual conté els p-valors.
...
També podem acceptar que totes les mostres provenen de variables normals.

```{block2,type="rmdimportant"}
Si efectuam $k$ contrastos de normalitat amb nivell de significació $\alpha$, tenim una probabilitat de cometre algun error de tipus I de $1-(1-\alpha)^k$. Per tant, com als contrastos posteriors per parelles, convé ajustar els p-valors a fi que el nivell de significació global sigui $\alpha$. 
```
...
```{r,eval=FALSE}
p.adjust(p-valors, method=...)
```
especificant a `method` el mètode d'ajust amb la mateixa sintaxi que a `pairwise.t.test` (per defecte, serà el de Holm).

Per exemple, per ajustar els p-valors de l'exemple del CO~2~ pel mètode de Bonferroni, i afegir el resultat al data frame amb els p-valors, faríem
...
Ja que hi som, comprovem l'homocedasticitat de l'Exemple  \@ref(exm:ANOVAterapgen), ara amb el test de Fligner-Killeen i emprant l'altra sintaxi possible perquè la vegeu
...
Si el test de Kruskal-Wallis permet rebutjar la igualtat de   mitjanes, i voleu determinar quines parelles són diferents,  disposau de la funció `pairwise.wilcox.test` que efectua contrastos per parelles de Mann-Whitney (amb  el paràmetre `paired=FALSE`). Té la mateixa sintaxi que `pairwise.t.test` i com en aquella funció, el mètode d'ajust dels p-valors s'hi entra amb el paràmetre `p.adjust.method`, el valor per defecte del qual  és el mètode de Holm.
...
on cada $X_{ij}$ és el valor del tractament $i$-èssim en l'individu corresponent del bloc $j$-èssim. Com en les taules emprades en l'ANOVA d'1 via, observau que no és la notació usual de les taules de dades, sinó la transposada: a $X_{{i}{j}}$, $i$ hi indica la **columna**,  el tractament, i $j$ hi indica la **filera**, és a dir, el bloc. D'aquesta manera, cada filera $(X_{1j},\ldots,X_{kj})$ representa un bloc.
...
* No hi hagi **interacció** entre els blocs i els tractaments. Això significa que l'efecte dels blocs i els tractaments és **additiu**:
...
$$
    ...
    * La diferència entre el consum energètic mitjà en córrer i en caminar és la mateixa a cada individu  de la mostra
    * La diferència entre el consum energètic mitjà en córrer i en pedalar és la mateixa a cada individu  de la mostra
    * La diferència entre el consum energètic mitjà en caminar i en pedalar és la mateixa a cada individu  de la mostra
...
L'ANOVA de blocs se basa en el **model** següent: per a tots $i=1,\ldots,k$ i $j=1,\ldots,b$
$$
X_{ij}=\mu+ (\mu_{i}-\mu) +(\mu_{\bullet j}-\mu) + E_{ij} 
$$
on:

* $X_{ij}$ indica el valor de la variable $X$ per al tractament $i$-èssim en el bloc $j$-èssim 

* $\mu_i-\mu$ representa la desviació de la mitjana de  $X$ en el nivell $i$-èssim  respecte de la mitjana global $\mu$ (l'**efecte del tractament $i$-èssim**)

* $\mu_{\bullet j}-\mu$ representa la desviació de la mitjana de  $X$ en el bloc $j$-èssim respecte de la mitjana global $\mu$ (l'**efecte del bloc $j$-èssim**)
...
* $\overline{X}_{i}$ és la mitjana mostral de la mostra del tractament $i$-èssim
$$
\overline{X}_{i}=\dfrac{\sum_{j=1}^b X_{ij}}{b}
$$
Estima la mitjana poblacional del tractament $i$-èssim, $\mu_i$


* $\overline{X}_{\bullet j}$ és la mitjana mostral de les mesures preses sobre el bloc $j$-èssim
$$
\overline{X}_{\bullet j}=\dfrac{\sum_{i=1}^k X_{ij}}{k}
$$
Estima la mitjana poblacional del bloc $j$-èssim, $\mu_{\bullet j}$
...
$$
Estima la mitjana poblacional global, $\mu$
...
```{block2,type="rmdnote"}
Si se satisfà el model de l'ANOVA de blocs
$$
X_{ij}=\mu+ (\mu_{i}-\mu) +(\mu_{\bullet j}-\mu) + E_{ij}  
$$
aleshores
...
$$
Veiem així que $SS_E$ és la suma dels quadrats de les estimacions d'aquests errors aleatoris per als individus de la nostra mostra.
...
A partir d'aquí, tot funciona com a l'ANOVA d'1 via. Al contrast 
$$
\left\{
\begin{array}{l}
...
\right.
$$
rebutjarem la hipòtesi nul·la si $SS_{Tr}$ és prou més gran que $SS_{E}$. Per mesurar-ho,
...
* A més, R calcula el **Quadrat mitjà dels blocs**, que nosaltres no farem servir però que esmentam per si de cas:
$$
MS_{Blocs}=\dfrac{SS_{Blocs}}{b-1}
$$

Si se satisfan les condicions necessàries que hem esmentat perquè l'ANOVA es pugui realitzar, es té que
$$
\begin{array}{l}
...
$$
i per tant $E(MS_{Tr})=E(MS_E)$, mentre que si $H_0$ no és certa
$$
\sum\limits_{i=1}^k (\mu_{i}-\mu)^2 > 0
$$
cas en el qual $E(MS_{Tr})>E(MS_E)$.
...
Totes aquestes dades se solen recollir en la **taula ANOVA** següent:
$$
\begin{array}{llllll}
\hline
\text{Origen de la}&\text{Graus de}&\text{Sumes de}&\text{Quadrats} & \text{Estadístic de}&\text{p-valor}\\[-0.5ex]
...
* *Conclusió*: Hem obtingut evidència estadística que no és veritat que les tres activitats tenguin el mateix consum energètic mitjà per km (ANOVA de blocs, p-valor 2·10^-8^)
...
```{example}
A l'experiment de l'Exemple \@ref(exm:esports) hem arribat a la conclusió que no totes les mitjanes són iguals. Emprem un contrast posterior per parelles amb l'ajust de Bonferroni per mirar quines són diferents.
...
*Conclusió*: Hem obtingut evidència estadística que les tres activitats tenen diferent consum energètic mitjà per km (ANOVA de blocs, test posterior de Bonferroni, p-valors ajustats inferiors a 0.0011)
...
Obtenim evidència estadística que les tres activitats tenen consums energètics mitjans diferents  (test de Friedman, test posterior per parelles de Wilcoxon amb ajust de Holm, p-valors ajustats 0.042).
...
* Indicarem amb $X_{ijk}$, $i=1,\ldots,a$, $j=1,\ldots,b$, $k=1,\ldots,n$, el valor de la variable $X$ en el $k$-èssim subjecte de la mostra corresponent  al nivell $i$-èssim del factor A i el nivell $j$-èssim del factor B
...
* $\mu_{i\bullet}$: la mitjana poblacional del nivell $i$-èssim del factor A

* $\mu_{\bullet j}$: mitjana poblacional del nivell  $j$-èssim  del factor B

* $\mu_{ij}$: mitjana poblacional de la combinació $(i,j)$ de nivells d'A i B


L'ANOVA de 2 vies se basa en el **model** següent: per a tots $i=1,\ldots,a$ i $j=1,\ldots,b$
$$
X_{ij}=\mu+ \alpha_i +\beta_j + (\alpha\beta)_{ij}+E_{ij} 
$$
on:

* $X_{ij}$ indica el valor de la variable $X$ sobre un individu que pertany al nivell $i$-èssim del factor A i al nivell $j$-èssim del factor B

* $\alpha_i=\mu_{i\bullet}-\mu$ representa la desviació de la mitjana de  $X$ en el nivell $i$-èssim del factor A respecte de la mitjana global $\mu$ (l'**efecte del tractament $i$-èssim del factor A**)

* $\beta_j=\mu_{\bullet j}-\mu$ representa la desviació de la mitjana de  $X$ en el nivell $j$-èssim del factor B respecte de la mitjana global $\mu$ (l'**efecte del tractament $j$-èssim del factor B**)

* $(\alpha\beta)_{ij}=\mu_{ij}-\mu_{i\bullet}-\mu_{\bullet j}+\mu$ representa l'**efecte de la interacció** entre el  nivell $i$-èssim del factor $A$ i el  nivell  $j$-èssim del factor $B$; si no hi hagués interacció, tendríem que
$$
\mu_{ij}-\mu=(\mu_{i\bullet}-\mu)+(\mu_{\bullet j}-\mu)
$$
(l'efecte dels dos tractaments se sumaria; la demostració la donam en un bloc de corbes perilloses més a baix) i aquesta contribució de la interacció seria 0.
...
* $\overline{X}_{i\bullet}=\dfrac{\sum_{j=1}^b\sum_{k=1}^n X_{ijk}}{bn}$: la mitjana mostral  del nivell $i$-èssim del factor A; estima $\mu_{i\bullet}$


* $\overline{X}_{\bullet j}=\dfrac{\sum_{i=1}^a\sum_{k=1}^n X_{ijk}}{an}$: la mitjana mostral  del nivell  $j$-èssim  del factor B estima $\mu_{\bullet j}$
...
Com a totes les ANOVA, els factors que s'empren per classificar la població convé que siguin això, factors (o, almenys, vectors de paraules, no vectors numèrics), al *dataframe* si volem després emprar la funció `aov` per fer l'ANOVA.
...
A més, de $\mu_{ij}-\mu_{i\bullet}-\mu_{\bullet j}+\mu=0$ deduïm que
$$
\mu_{ij}-\mu=\mu_{i\bullet}+\mu_{\bullet j}-2\mu=(\mu_{i\bullet}-\mu)+(\mu_{\bullet j}-\mu)
$$
com afirmàvem en parlar del model de l'ANOVA de 2 vies.
...
> La variabilitat total descompon en la suma de la variabilitat de les mitjanes del factor A més la variabilitat de les mitjanes del factor B més la variabilitat deguda a la interacció més la variabilitat de les dades dins cada combinació de tractaments, un de cada factor.
...
Com a les altres ANOVA, compararem la variabilitat de les mitjanes amb la variabilitat dels errors. Per això, emprarem els quadrats mitjans següents:
...
```{block2,type="rmdcaution"}
Com fins ara, els graus de llibertat de cada estadístic de contrast F són els denominadors emprats en les definicions dels quadrats mitjans del numerador i el denominador.
````
...
* Hi ha diferència en la mitjana del GSI segons el foto-període?
$$
\begin{array}{l}
...
\end{array}
$$
    *Conclusió*: Hem trobat evidència estadística que el GSI mitjà és diferent segons el foto-període (ANOVA de blocs, p-valor 0.0003)
...
\end{array}
$$
    *Conclusió*: Hem trobat evidència estadística que el  GSI mitjà és diferent segons la temperatura (ANOVA de blocs, p-valor 2·10^-5^)


* Hi ha diferència en la mitjana del GSI segons les combinacions de foto-període i temperatura?
$$
\begin{array}{l}
...
\end{array}
$$
    *Conclusió*: Hem trobat evidència estadística que no tots els GSI mitjans per a les combinacions de foto-període i temperatura són iguals (ANOVA de blocs, p-valor 9·10^-6^)




* Hi ha interacció entre el foto-període i la temperatura?
$$
\begin{array}{l}
...
\end{array}
$$
    *Conclusió*: No hem trobat evidència estadística  que hi hagi interacció entre el foto-període i la temperatura (ANOVA de blocs, p-valor 0.051)
...
1. Per contrastar si els valors mitjans d'una variable quantitativa sobre diverses poblacions són tots diferents o no
1. Per contrastar si els valors mitjans d'una variable quantitativa sobre diverses poblacions són tots iguals o no
1. Per contrastar si entre els valors mitjans d'una variable quantitativa sobre diverses poblacions n'hi ha exactament dos que siguin diferents o no.
1. Per contrastar si les mitjanes mostrals  d'una variable quantitativa sobre diverses mostres extretes de diferents subpoblacions són totes iguals o no
1. Per contrastar si les mitjanes mostrals  d'una variable quantitativa sobre diverses mostres extretes de diferents subpoblacions són totes diferents o no
1. Per contrastar si les variàncies d'una variable quantitativa sobre diverses poblacions són totes diferents o no
1. Per contrastar si les variàncies d'una variable contínua sobre diverses poblacions són totes iguals o no
...
**()** Quina és la hipòtesi alternativa en el contrast que efectuam amb una ANOVA?

1. Que totes les mitjanes mostrals  són diferents.
1. Que totes les mitjanes poblacionals  són diferents.
1. Que hi ha alguna parella de mitjanes mostrals  que són diferents
1. Que hi ha alguna parella de mitjanes poblacionals  que són diferents
1. Que hi ha exactament una parella de mitjanes mostrals  que són diferents
1. Que hi ha exactament una parella de mitjanes poblacionals  que són diferents
...
**()** Un contrast d'homogeneïtat on comparam la probabilitat d'èxit d'una variable Bernoulli sobre diferents poblacions (per veure si sempre és la mateixa o no) es pot entendre com un contrast d'igualtat de mitjanes. Si les mostres són prou grans, podem emprar una ANOVA per efectuar-lo?
...
**()** Què seria un error de tipus II en una ANOVA?

1. Concloure que totes les mitjanes mostrals són iguals quan en realitat són totes diferents
1. Concloure que hi ha una parella de mitjanes mostrals que són iguals quan en realitat 1. són totes diferents
1. Concloure que totes les mitjanes mostrals són iguals quan en realitat n'hi ha alguna parella que són diferents
1. Concloure que totes les mitjanes poblacionals són iguals quan en realitat són totes diferents
1. Concloure que hi ha una parella de mitjanes poblacionals que són iguals quan en realitat són totes diferents
1. Concloure que totes les mitjanes poblacionals són iguals quan en realitat n'hi ha alguna parella que són diferents


**()** Per poder efectuar una ANOVA d'1 via, cal que totes les mostres tenguin la mateixa mida?
...
**()** Quines de les condicions següents són necessàries per poder efectuar una ANOVA d'1 via?

1. Les variables poblacionals han de ser normals
1. Les variables poblacionals han de ser normals o les mostres han de ser grans
1. Les variàncies de les mostres han de ser totes iguals
1. Les variàncies de les poblacions definides pels nivells han de ser totes iguals
1. Les variàncies de les poblacions definides pels nivells no poden ser totes diferents
1. Les distribucions de les poblacions definides pels nivells han de ser totes la mateixa (han de ser de la mateixa família i tenir els mateixos paràmetres)
1. Les mostres han de ser independents
1. Les mostres han de ser aparellades
1. Les mostres no han de ser aparellades
...
**()** Per contrastar si tres somnífers tenen el mateix efecte, prenc 10 persones i durant 3 dies consecutius a cada una li don cada dia un somnífer diferent, en un ordre escollit a l'atzar, a l'hora d'anar a dormir i mesur el temps que tarda en dormir-se. Aquest experiment, te el disseny experimental d'una ANOVA d'1 via?
...
**()** Si en efectuar una ANOVA d’1 via va i resulta que la identitat de la suma de quadrats no se satisfà, què ha passat? 

1. Un error de tipus I
1. Un error de tipus II
1. Un error de càlcul




**()** Quines de les afirmacions següents sobre l'ANOVA d'1 via són correctes? Marcau totes les respostes correctes.
 
1. La variància total és igual a la suma de les variàncies dels tractaments més la suma de la variància dels residus.
1. La suma total de quadrats és igual a la suma dels quadrats dels tractaments més la suma dels quadrats dels residus. 
1. La variància de les dades és igual a la suma del quadrat mitjà dels  tractaments més el quadrat mitjà dels residus.
1. La suma dels dos graus de llibertat de la distribució $F$ de l'estadístic de contrast és igual al nombre total de dades.
1. Les mostres dels diferents tractaments poden ser independents o aparellades.
1. Les mostres dels diferents tractaments poden tenir mides diferents. 
1. Cap de les respostes anteriors és correcta.
 


**()**  Hem realitzat un ANOVA de 1 via. Les dues primeres columnes de la taula resultant són:
$$
...
\end{array}
$$
Quines de les afirmacions següents són vertaderes? Marcau totes les respostes correctes.

 
1. $SS_{Total}= 215.1+147.3=362.4$
1. El nombre de nivells del factor d'interès és 3
1. El nombre total de dades ha estat 24
1. El nombre total de dades ha estat 25 
1. El p-valor és més petit que 0.05 
1. Cap de les altres respostes és correcta



**()** Si volem realitzar un contrast posterior per parelles després d'una ANOVA d'1 via on hem comparat 5 tractaments, i empram el mètode d'ajust de Bonferroni, per quin valor hem de multiplicar els p-valors dels contrastos individuals?
...
**()** S'ha realitzat un assaig clínic dirigit a comparar l'efectivitat de tres tractaments alternatius en pacients cirròtics, provant-los sobre tres grups independents de malalts. La variable de resposta ha estat el nivell de transaminasses, que és contínua i de distribució asimètrica a la dreta. Quina de les proves estadístiques  següents és la més adequada per contrastar si els tres tractaments tenen la mateixa efectivitat o no?:
 
1. 3 proves t per a mostres independents, una per a cada parell de tractaments
1. Una anàlisi de la variància 
1. Un contrast d'homogeneïtat
1. Un test de Kruskal-Wallis 
1. 3 proves de Mann-Whitney, una per a cada parell de tractaments



**()** En una ANOVA de blocs, per calcular el p-valor hem emprat una distribució $F$ amb 3 i 21 graus de llibertat. De quina mida era la mostra completa que hi hem emprat?
...
1. De 32 individus 
1. De 63 individus
1. No ho podem saber a partir d'aquests nombres de graus de llibertat
1. Cap de les altres respostes és correcta


**()**  Hem realitzat una ANOVA de blocs. Hi comparàvem 3 tractaments i hem emprat 10 blocs; les dades no han estat totes iguals. Si haguéssim analitzat les mateixes dades, de manera incorrecta, amb una ANOVA d'una via, quines de les afirmacions següents segur que serien vertaderes? Marcau totes les respostes correctes.
 
1. La suma de quadrats total hagués estat més gran
1. La suma de quadrats dels errors hagués estat més petit 
1. La suma de quadrats dels errors hagués estat més gran
1. El quadrat mitjà dels errors hagués estat més petit 
1. El quadrat mitjà dels errors hagués estat més gran 
1. El segon nombre de graus de llibertat de l'estadístic $F$ emprat per calcular el p-valor seria més petit 
1. El segon nombre de graus de llibertat de l'estadístic $F$ emprat per calcular el p-valor seria més gran
1. Cap de les altres respostes és correcta



**()** Respecte al test de Friedman, quines de les afirmacions següents són correctes? Marcau totes les respostes correctes.
 
1. És un test no paramètric.
1. S'utilitza per comparar les medianes de tres o més poblacions a partir de mostres independents. 
1. S'utilitza per comparar les medianes de tres o més poblacions a partir de mostres aparellades. 
1. Si el realitzam sobre 2 poblacions, és equivalent al test de Wilcoxon 
1. Si el realitzam sobre 2 poblacions, és equivalent al test de Mann-Whitney 
1. Requereix que les mostres provenguin de distribucions normals.
1. Requereix que les mostres provenguin de distribucions NO normals.




**()** En una ANOVA de 2 vies, en quantes fonts de variabilitat independents descompon la variabilitat total?
...
**()** Què seria un error de tipus I en un contrast de no interacció en una ANOVA de dues vies? Marcau l'única resposta correcta. 

1. Concloure que hi ha interacció entre tots els nivells d'un factor i tots els nivells de l'altre factor quan no és veritat
1. Concloure que hi ha interacció entre algun nivell d'un factor i algun nivell de l'altre factor quan no és veritat 
1. Concloure que no hi ha interacció entre cap nivell d'un factor i cap nivell de l'altre factor quan sí que n'hi ha
1. Concloure que el diagrama d'interacció mostra que hi ha interacció quan en realitat no ho mostra
1. Concloure que el diagrama d'interacció mostra que no hi ha interacció quan en realitat sí que ho mostra
1. Cap de les altres respostes és correcta.
 


**()** Respecte al test de Kruskal-Wallis, quines de les afirmacions següents són correctes? Marcau totes les respostes correctes.
 
1. És un test no paramètric.
1. S'utilitza per comparar les mitjanes (en realitat, les medianes) de tres o més poblacions a partir de mostres independents. 
1. S'utilitza per comparar les mitjanes (en realitat, les medianes) de tres o més poblacions a partir de mostres aparellades. 
1. Si el realitzam sobre 2 poblacions, és equivalent al test de Wilcoxon
1. Si el realitzam sobre 2 poblacions, és equivalent al test de Mann-Whitney  
1. Requereix que les mostres provenguin de distribucions normals.
1. Requereix que les mostres provenguin de distribucions NO normals.
...
**()** Si volem realitzar un contrast posterior per parelles després d'una ANOVA d'1 via on hem comparat 4 tractaments, i empram el mètode d'ajust de Bonferroni, per quin valor hem de multiplicar els p-valors dels contrastos individuals?
...
**()**  Hem realitzat una ANOVA de blocs. Les dues primeres columnes de la taula resultant són:
$$
\begin{array}{|@{}l@{}|l@{}|l@{}|l@{}|l|l|}
\hline
\text{Font de}&\text{Graus de}\hphantom{x} &\text{Suma de}\hphantom{x}&\text{Quadrats}\hphantom{x} &\text{Estadístic}\hphantom{x} & \text{p-valor}\\
...
1. $SS_{Total}=6.75+8.32=15.07$
1. El quadrat mitjà dels tractaments és aproximadament 2.25 
1. El nombre total de dades ha estat 30
1. El nombre total de dades ha estat 40 
1. Amb $\alpha=0.05$, podem rebutjar la hipòtesi nul·la que les mitjanes dels tractaments són totes iguals 
1. Cap de les altres respostes és correcta



**()** En una ANOVA de blocs hem obtingut un p-valor molt més petit que el nivell de significació. Quina és la conclusió correcta? Marcau només una resposta. 

1. Que hi ha algun parell de mitjanes mostrals dels tractaments que han estat diferents.
1. Que hi ha algun parell de mitjanes  poblacionals dels tractaments que són diferents. 
1. Que hi ha algun parell de mitjanes  poblacionals dels blocs que són diferents. 
1. Que hi ha algun parell de mitjanes  poblacionals dels blocs que han estat diferents. 
1. Que les mitjanes mostrals dels tractaments han estat totes diferents.
1. Que les mitjanes poblacionals dels tractaments són totes diferents. 
1. Que les mitjanes mostrals dels blocs han estat totes diferents.
1. Que les mitjanes poblacionals dels blocs són totes diferents. 
1. Cap de les respostes anteriors és correcta.
 


**()** Quines de les afirmacions següents sobre l'ANOVA de blocs són correctes? Marcau  totes les correctes:

1. Rebutjarem la hipòtesi nul·la si la suma de quadrats dels residus és prou gran.
1. Rebutjarem la hipòtesi nul·la si la suma de quadrats dels blocs és prou gran.
1. Rebutjarem la hipòtesi nul·la si la suma de quadrats dels tractaments és prou gran. 
1. La suma de quadrats total és igual a la suma de quadrats dels tractaments més la suma dels quadrats dels residus.
1. La suma de quadrats total és igual a la suma de quadrats dels tractaments més la suma dels quadrats dels blocs.
 



**()** Què significa que hi hagi interacció entre els dos factors en una ANOVA de 2 factors?
 
1. Que almenys en un dels factors hem trobat diferències estadísticament significatives 
1. Que hem trobat  en els dos factors diferències estadísticament significatives entre les mitjanes
1. Que les diferències entre les mitjanes poblacionals  dels nivells d'un dels factors depenen del nivell de l'altre factor 
1. Que les diferències entre les mitjanes poblacionals  dels nivells d'un dels factors són independents del nivell de l'altre factor
1. Que els dos factors no són variables independents



**()** En un assaig clínic es comparen 3 tractaments (placebo, tractament establert i un tractament nou) sobre 3 grups de pacients escollits de manera independent. Acceptam que la variable resposta té distribució normal i amb la mateixa variància sota cada un dels tractaments. Quina de les proves estadístiques següents és la més adequada per comparar aquesta resposta?
 
1. Tres tests t de mitjanes
...
**()** En un assaig clínic es comparen 3 tractaments (placebo, tractament establert i un tractament nou) sobre 3 grups de pacients escollits de manera independent. La variable resposta és contínua i la seva variància no depèn del tractament, però sota el tractament nou presenta una distribució molt asimètrica. Quina de les proves estadístiques següents és la més adequada per comparar aquesta resposta?
...
**()** Un assaig clínic avalua 5 pautes terapèutiques en grups de pacients hipertensos dels dos sexes, 10 grups en total (un per cada combinació de tractament i sexe). Es creu que el sexe influeix en la resposta als tractaments. Per comparar les xifres de pressió arterial observades en els 10 grups als 6 mesos de tractament, quina de les proves estadístiques següents és la més adequada?:
 
1. Una ANOVA d'una via 
1. 45 tests t per a mostres independents
1. Un contrast de correlació de Pearson
1. Un test khi-quadrat
1. Una ANOVA  de dues vies 



**()** Un assaig clínic avalua 5 pautes terapèutiques en pacients hipertensos, aplicant cada tractament a un grup diferent escollit de manera independent dels altres. Per contrastar si els cinc tractaments tenen la mateixa efectivitat en termes de percentatge de curació de la hipertensió als 6 mesos, quina de les  proves estadístiques següents és la més adequada?:
 
1. 10 tests t per a mostres independents
1. Un test de Wilcoxon
1. Una anàlisi de la variància
