...
Prenent valors esperats als dos costats de la igualtat $Y|x=\mu_{Y|x}+ E_x$ obtenim que $\mu_{Y|x}=\mu_{Y|x}+ \mu_{E_x}$ i per tant que $\mu_{E_x}=0$. Així doncs, aquest model implica que els valors esperat de les variables error $E_x$ són tots 0.
...
$$
\frac{{s}_{xy}}{{s}_x^2}=\frac{\frac{\sum_{i=1}^n (x_i-\overline{x})(y_i-\overline{y})}{n}}{\frac{\sum_{i=1}^n (x_i-\overline{x})^2}{n}}=\frac{\frac{\sum_{i=1}^n (x_i-\overline{x})(y_i-\overline{y})}{n-1}}{\frac{\sum_{i=1}^n (x_i-\overline{x})^2}{n-1}}=
...
$$
La moralitat d'aquesta història és que no arrodoniu fins al resultat final.
```
...
$$
Segons aquest model, l'augment de 1 g de sal consumida implica un augment mitjà de `r round(b1.sal,1)` mm Hg de pressió arterial.
...
```{example,Galton}
Anem a estimar la recta de regressió de les alçades dels fills en funció de les dels pares emprant les dades recollides per Galton, que formen el dataframe `Galton` del paquet **HistData**.
```
...
$$
Segons aquest model, l'augment de 1 polzada (2.54 cm) en l'alçada mitjana dels pares implica un augment esperat de `r round(b1.Galton,3)` polzades (`r round(b1.Galton*2.54,1)` cm) de l'alçada del fill.
...
El fet que la pendent d'aquesta recta sigui més petita que 1 és el que dóna l'efecte de "regressió a la mediocritat" que observà Galton. Anem a determinar per a quines alçades mitjanes dels pares esperam que els fills siguin més baixos que ells. Si resolem la desigualtat
$$
X\geq \widehat{Y}=  `r round(b0.Galton,3)`+`r round(b1.Galton,3)` X
$$
obtenim
$$
X\geq \frac{`r round(b0.Galton,3)`}{1-`r round(b1.Galton,3)`}=`r round(b0.Galton/(1-b1.Galton),2)`
$$
i això ens diu que si l'alçada mitjana dels pares és més gran que `r round(b0.Galton/(1-b1.Galton),2)` polzades, uns `r round(b0.Galton/(1-b1.Galton)*2.54/100,2)` m, aleshores l'alçada esperada dels fills és més petita que la  mitjana dels pares, i que, pel contrari, l'alçada mitjana dels pares està per davall dels `r round(b0.Galton/(1-b1.Galton)*2.54/100,2)` m, aleshores esperam que els  fills siguin més alts que els pares.
...
D'entrada, pot ser útil saber  que el vector `Residuals` (que s'obté amb el sufix `$residuals`) conté el vector dels errors $(e_i)_i$. Comprovem-ho amb les dades de l'Exemple \@ref(exm:edats1)
...
* $SS_{Total} =\sum\limits_{i=1}^n(y_i-\overline{y})^2$: és la **Suma Total de Quadrats** i representa la **variabilitat dels valors observats $y_i$**. Fixau-vos que
$$
...
És a dir, el coeficient de determinació $R^2$ és la fracció de la variabilitat de les $y_i$ que queda explicada per la variabilitat de de les estimacions $\widehat{y}_i$.
...
R calcula $R^2$ en el `summary(lm( )`: és el valor `Multiple R-squared` a la penúltima línia de la seva sortida:
...
Calculem ara $R^2=SS_R/SS_{Total}$ i comprovem que coincideix amb el valor que dóna R i amb el quadrat de la correlació de Pearson dels vectors de quantitats de sal i tensions:
```{r}
R2=SS.R/SS.Tot
R2
...
Resulta que si se satisfan les condicions demanades sobre les variables $E_{x_i}$, coneixem els errors típics dels estimadors $b_1$ i $b_0$ i uns estadístics associats a aquests estimadors segueixen lleis t de Student que permeten calcular intervals de confiança per a $\beta_0$ i $\beta_1$. En concret:
...
$$
l'interval de confiança del valor esperat serà més estret que el del valor sobre un individu concret. Això reflecteix el fet que, naturalment, hi ha molta més incertesa en saber que val la $Y$ sobre un individu concret que en saber quin és el valor mitjà de $Y$ sobre tots els individus que tenguin el mateix valor de $X$ que aquest individu concret.
...
Per exemple, a l'Exemple \@ref(exm:sal) hem obtingut l'IC 95% per a $\beta_1$   $[5.764,7.164]$. Com que no conté el 0, concloem (amb un nivell de significació de 0.05) que $\beta_1\neq 0$.
...
els `t value` són els valors de l'estadístic de contrast dels contrastos bilaterals amb hipòtesi nul·la $H_0:$ "coeficient $=0$", i els p-valors `Pr(>|t|)` són els d'aquests  contrastos. Com veiem, podem rebutjar que $\beta_1=0$ (i que $\beta_0=0$).
...
\right.
$$
no sigui indeterminat. Direm $b_0,b_1,\ldots,b_k$ a les estimacions dels paràmetres $\beta_0,\beta_1,\ldots,\beta_k$ a partir d'una mostra, i per escurçar escriurem $\underline{x}_i$ per indicar $(x_{i1},x_{i2},\ldots,x_{ik})$.
...
$$
\left(\begin{array}{c} b_0 \\ b_1 \\ \vdots \\ b_k\end{array}\right)=\left(\mathbf{X}^t\cdot \mathbf{X} \right)^{-1}\cdot \left(\mathbf{X}^t \cdot \mathbf{y}\right)
$$
...
R= \sqrt{R^2}
$$
i així també se te que **el coeficient de determinació múltiple és el quadrat del coeficient de correlació múltiple**.
```
...
Es considera que una regressió lineal múltiple per mínims quadrats és "millor" que una altra quan té el coeficient de determinació ajustat més gran. Això només té interés per a regressions amb diferents nombres de variables independents i la mateixa mostra d'individus, perquè fixats $n$ i $k$, la funció
$$
...
Com al cas simple, la sortida de `summary(lm( ))` dóna el valor de la $S$ (és a dir, l'arrel quadrada de $S^2$, que  per tant estima la desviació típica comuna de les variables error) com al `Residual standard error` 
...
Els podem calcular "a mà" a partir de les estimacions dels errors típics que dóna `summary(lm( ))`. Per exemple, l'IC 95% per a $\beta_1$ seria
```{r}
n=9
k=4
b1=lm(y~x1+x2+x3+x4)$coefficients[2]
Error.Tip=summary(lm(y~x1+x2+x3+x4))$coefficients[2,2]
IC=b1+qt(0.975,n-k-1)*Error.Tip*c(-1,1)
IC
```
...
$$
i, com a les altres ANOVA, l'estadístic de contrast $F$ és
$$
F=\frac{MS_R}{MS_E}
$$
i satisfà que, si la hipòtesi nul·la és vertadera (i els errors satisfan les condicions establertes al començament de la secció anterior),  segueix una llei F de Fisher-Snedecor amb $k$ i $n-k-1$ graus de llibertat i té valor proper a 1, de manera que se pren com a p-valor
$$
...
L'estadístic de contrast i el p-valor d'aquesta ANOVA els podeu trobar a la darrera filera de la sortida `summary(lm( ))`; són el `F-statistic` (i a més us diu els graus de llibertat, `DF`, de la seva distribució) i el `p-value`:
...
```{block2,type="rmdnote"}
Una altra opció  és emprar la funció `glance` del paquet **broom**, que aplicada a un objecte dóna un data frame (de fet, una adaptació del concepte de data frame per a l'anàlisi de dades massives, un **tibble**) amb els seus continguts més importants; vaja, que us permet "donar una ullada" a l'objecte.
```

```{r}
library(broom)
glance(summary(lm(y~x1+x2+x3+x4)))
glance(summary(lm(y~x1+x2+x3+x4)))[1,5] # El p-valor
```
