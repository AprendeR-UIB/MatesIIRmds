...
$$
Sovint també l'indicarem amb $\mu_X$.
...
$$
És a dir, és la mitjana del quadrat de la diferència entre $X$ i la seva mitjana $\mu_X$. També l'indicarem amb $\sigma_X^2$.
...
$$
També mesura la dispersió dels valors de $X$ respecte de la mitjana. Sovint l'indicarem amb $\sigma_X$.
...
```{block2,type="rmdcaution"}        
No confongueu la proporció de persones que donen positiu en el test PCR de COVID-19 amb la proporció de persones que tenen la COVID-19.
```
...
* En una aula hi ha 5 homes i 45 dones. Triam 10 estudiants, un rere l'altre i sense repetir-los, per fer-los una pregunta. Cada elecció és independent de les altres. Comptam quants homes hem interrogat. 
    ...
    Aquesta variable és $H(5,45,10)$. El factor de població finita en aquesta cas no és aproximadament 1: dóna
$$
...
Com en el cas discret, també l'indicarem de vegades amb $\mu_X$. 
...
\sigma(X)^2=E(X^2)-\mu_X^2.
$$
També l'indicarem de vegades amb $\sigma_X^2$.
...
\sigma(X)=+\sqrt{\sigma(X)^2}
$$
i també l'indicarem de vegades amb $\sigma_X$.
...
```{block2,type="rmdnote"}
I és clar, si $X$ només pot prendre un valor, aleshores ja no és contínua, sino discreta. Per tant, per conveni, d'ara endavant suposarem que **les nostres variables aleatòries contínues tenen variància no nul·la**.
```
...
```{block2,type="rmdnote"}
Ara que sabem més coses de la normal, a l'Exemple \@ref(exm:exhiperhipo) ens haguéssim pogut estalviar la meitat de la feina. Diguem $X$ a la variable aleatòria la pressió arterial, en mm Hg, d'un home d'entre 16 i 24 anys. Ens diuen que $X$ és $N(124,13.7)$. 
...
```{block2,type="rmdnote"}
Si $a>1$, $\log_a(X)=\ln(X)/\ln(a)$ i per tant, pel Teorema \@ref(thm:comblinnormals).1, si $X$ és log-normal, el seu logaritme en qualsevol base $a>1$ és normal.
```
