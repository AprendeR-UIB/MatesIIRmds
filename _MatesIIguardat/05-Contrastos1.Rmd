...
*  *H*~0~ sempre ha de significar "no hi ha diferència" i s'ha de definir formalment mitjançant una igualtat
...
6.  Si aquesta probabilitat és molt petita, ho consideram evidència que *H*~1~ és vertadera
...
* En molts casos, *la probabilitat d'obtenir exactament el que hem obtengut pot ser molt petita, independentment del que hàgim obtengut*. Per exemple, suposem que llançam la moneda  10000 vegades i obtenim 5000 cares. Si la moneda és equilibrada, el nombre de cares seguirà una distribució binomial B(10000,0.5) i la probabilitat d'obtenir 5000 cares serà `dbinom(5000,10000,0.5)`=`r round(dbinom(5000,10000,0.5),4)`, ben petita, però clarament si la meitat de llançaments donen cara, no podem tenir mai evidència que la moneda estigui trucada.  
...
*  **Significa** que, si *H*~0~ és vertadera, la probabilitat que l'estadístic de contrast prengui sobre una mostra un valor tan extrem o més que el que hem obtengut és 0.03 
...
En aquesta situació, si *H*~0~ és vertadera, és a dir, si la mitjana de $X$ és $\mu_0$, sabem que
$$
...
> Rebutjarem *H*~0~ a favor de *H*~1~ si aquest *estadístic de contrast* $T$ pren un valor "molt gran" sobre la mostra, és a dir, si $\overline{X}$ és "molts errors típics" més gran que $\mu_0$.
...
> Rebutjarem *H*~0~ si $T_0>t_{n-1,1-\alpha}$
...
\right.
$$
Si $\alpha=0.05$ i $n=20$, el llindar a partir del qual rebutjam *H*~0~ és $t_{n-1,1-\alpha}=t_{19,0.95}=\texttt{qt(0.95,19)}=1.73$.
...
> Rebutjarem *H*~0~ quan el  p-valor sigui més petit que $\alpha$
...
T=\frac{\overline{X}-\mu_0}{{\widetilde{S}_X}/{\sqrt{n}}}
$$
té distribució t de Student amb $n-1$ graus de llibertat, $t_{n-1}$, se'n diu un **test t**. Com explicarem al pròxim tema, podem emprar un test t per contrastar una mitjana quan la variable poblacional és normal i també quan la variable poblacional no és normal però la mostra és prou gran (posem, $n\geqslant 40$).
```
...
> Rebutjarem *H*~0~ si  $T_0< t_{n-1,\alpha}$  
> Rebutjarem *H*~0~ si el p-valor és més petit que $\alpha$
...
>  Rebutjarem *H*~0~ si $|T_0|>t_{n-1,1-\alpha/2}$
...
* Si $H_1:\mu> \mu_0$:
    ...
    * Rebutjam *H*~0~ si $T_0>t_{n-1,1-\alpha}$
    * El p-valor és $P(T\geqslant T_0)$ i rebutjam *H*~0~ si aquest p-valor és més petit que $\alpha$
    
* Si $H_1:\mu< \mu_0$:

    * Rebutjam *H*~0~ si $T_0< t_{n-1,\alpha}$
    * El p-valor és $P(T\leqslant T_0)$ i rebutjam *H*~0~ si aquest p-valor és més petit que $\alpha$
    
* Si $H_1:\mu\neq  \mu_0$:

    * Rebutjam *H*~0~ si $|T_0|>t_{n-1,1-\alpha/2}$
    * El p-valor és $2P(T\geqslant |T_0|)$ i rebutjam *H*~0~ si aquest p-valor és més petit que $\alpha$
...
**(15)** En un examen considerat com un contrast (marcau totes les respostes
correctes)
