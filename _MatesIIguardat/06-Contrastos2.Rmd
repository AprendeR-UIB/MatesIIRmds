...
* $X_1,X_2$ dues variables aleatòries qualssevol de mitjanes $\mu_1$, $\mu_2$ i en prenem mostres aleatòries simples de mides $n_1$, $n_2$ grans (diguem que **totes dues** de mida com a mínim 30)
...
$$
que, quan $\mu_D=(\mu_1-\mu_2)=0$, té (aproximadament, si $X_1,X_2$ no són normals però la $n$ és gran) distribució $t_{n-1}$.
...
$$
que, quan $\mu_1=\mu_2$, té distribució (aproximadament, si $X_1,X_2$ no són normals però $n_1$ i $n_2$ són totes dues grans) $t_{n_1+n_2-2}$
...
$$
que, quan $\mu_1=\mu_2$, té distribució (aproximadament, si $X_1,X_2$ no són normals però $n_1$ i $n_2$ són totes dues grans) $t_{\nu}$ amb
$$
...
* A més, com que l'IC 95% per a la temperatura mitjana del cos humà que hem obtingut  va de 98.2 a 98.4 (36.78 a 36.89^o^ C), hem trobat evidència amb aquest nivell de confiança que aquesta temperatura mitjana és de fet (lleugerament) inferior 98.6^o^ F
...
Si, per exemple,  haguéssim dibuixat abans del contrast un diagrama de caixes de les temperatures separades per sexe, hauríem vist que era impossible obtenir evidència que les dones tenen temperatura mitjana inferior als homes, i no hauria fet falta continuar.
...
* $X_{ds}$: "Prenem un nadó de 2 setmanes i mesuram el seu nivell de triglicèrids en plasmaen ng/dl", amb mitjana $\mu_{cf}$
...
* $\log(X_{ds})$: "Prenem un nadó de 2 setmanes i  calculam el logaritme del seu nivell de triglicèrids en plasma", amb mitjana $\mu_{logds}$
...
4. Si $H_0$ vertadera, és a dir, si $\mu=\mu_0$, per la simetria de $X$ al voltant de $\mu$ esperam que $T_+\approx T_-$. Per tant:
    ...
    * Si $T_+$ és molt petit, és evidència que $\mu <\mu_0$  
        ...
        En efecte, si $T_+$ és molt més petit que $T_-$, bàsicament significa que hi ha més valors a l'esquerra de $\mu_0$ que a la dreta, i per tant la mediana poblacional (que és el valor que deixa la meitat de la població a l'esquerra i l'altra meitat a la dreta) ha d'estar a l'esquerra de $\mu_0$.
...
6. I ara miraríem si $T_+$ és prou més petit que $T_-$ per que sigui evidència estadísticament significativa de que $\mu <\mu_0$. Això ja no ho podem fer a mà.
...
```{example}
Suposem que tenim una mostra aleatòria simple d'una variable $X$ normal de mida 25, i ha donat 
...
* Si $H_{1}:\sigma_1^2>\sigma_2^2$, el p-valor del contrast és $P(F\geq F_0)$

* Si $H_{1}:\sigma_1^2<\sigma_2^2$, el p-valor del contrast és $P(F\leq F_0)$
...
$$
\begin{array}{l}
2\min\big\{P(F_{n_d-1,n_h-1} \geq F_0), P(F_{n_d-1,n_h-1}\leq F_0)\big\}\\
...
Obtenim el mateix p-valor que abans. L'interval de confiança que dóna aquesta funció és *per al quocient de les variàncies*. Aleshores, com que l'IC 95% conté l'1, no podem rebutjar amb un nivell de significació del 5% que $\sigma_d^2/\sigma_h^2=1$, és a dir, que $\sigma_d^2=\sigma_h^2$. Podem ampliar la conclusió:
...
* Si $H_{1}:p\neq p_{0}$, el p-valor és la probabilitat que $S$ estigui tan o més allunyada que $S_0$ del nombre d'èxits que esperaríem si $H_0$ fos vertadera, que és $p_0\cdot n$. Per tant, és
$$
\text{p-valor}=P(|S-p_0\cdot n|\geq |S_0-p_0\cdot n|)
$$
...
$$
\text{p-valor}=\sum_{k\ \mathrm{tal\ que}\atop P(S=k)\leq P(S=S_0)} P(S=k)
$$
...
```{r,eval=FALSE}
binom.test(S, n, p=..., alternative=..., conf.level=...)
```

on `S` indica al nombre d'èxits a la mostra, `n` la mida de la mostra, `p` la probablitat que es contrasta, i els altres dos paràmetres signifiquen el mateix que a les altres funcions explicades fins ara. L'interval de confiança que dóna en els contrastos bilaterals és el de Clopper-Pearson. 
...
$$
\text{p-valor}=\sum_{P(S=k)\leq P(S=S_0)} P(S=k)
$$
Vegem-ho. Direm `PB` al vector que dóna els valors de la densitat de $S$ sobre $k=0,\ldots,30$, que són tots els possibles valors de $k$. Aleshores, els $k$ tals que $P(S=k)\leq P(S=S_0)$ són els que satisfan `PB<=dbinom(1,30,0.1)` (o també `PB<=PB[2]`, perquè $P(S=1)$ és la segona entrada del vector `PB`; la primera és $P(S=0)$). Per tant, la suma de les seves probabilitats és:
```
...
```{block2,type="rmdnote"}
Aquest test s'anomena **prop-test**, o també **test** $\chi^2$ perquè és un cas particular d'un test $\chi^2$ que veurem al proper tema. De fet, com en el cas d'una proporció, en la seva implementació en R no s'hi empra $Z$ sinó $Z^2$, que té distribució aproximadament $\chi^2_1$.
```
...
Com és aquesta mostra, per conglomerats o estratificada?
```
...
*Valor de l'estadístic*: Teim que $\widehat{p}_1=0.2$, $\widehat{p}_2=0.24$, $n_1=100$, $n_2=50$, $E=20+12=32$ i per tant
$$
z_0=\frac{0.2 -0.24}{
...
* Si $H_1:p_1>p_2$, el p-valor és $P(N_{1,1}\geq n_{1,1})$, on, recordem, $N_{1,1}\sim H(E,F,n_{1})$

* Si $H_1:p_1<p_2$, el p-valor és $P(N_{1,1}\leq n_{1,1})$
...
$$
\sum_{k\ \mathrm{tal\ que}\atop P(N_{1,1}=k)\leq P(N_{1,1}=n_{1,1})} P(N_{1,1}=k)
$$
...
Vegem. Si jugau 1 euro a $A$, esperau guanyar $R\cdot P(A)=R\cdot m\cdot P(A^c)$, i si jugau 1 euro a $A^c$ esperau guanyar $1\cdot P(A^c)$. Per tant, si aquests guanys esperats han de ser iguals, ha de passar $R\cdot m=1$, és a dir $R=1/m$. 
...
* Però el fet que $\text{Odds}(A)=1.5\cdot \text{Odds}(B)$ **no implica** que $P(A)=1.5P(B)$. De fet, d'aquest valor de $OR(A,B)$ no podeu deduir el valor de $P(A)/P(B)$. Intentau-ho si no ens creieu.
...
$$
\begin{array}{l}
P(|N_{1,1}-21.333|\geq 1.333)=P(N_{1,1}\leq 20\text{ o }N_{1,1}\geq 22.666)\\
\qquad= P(N_{1,1}\leq 20\text{ o }N_{1,1}\geq 23)\qquad \text{(perquè $N_{1,1}\in \mathbb{N}$)}\\
\qquad= P(N_{1,1}\leq 20)+P(N_{1,1}\geq 23)\\
...
```{r}
fisher.test(M.A,alternative="two.sided")$p.value
```
...
Això permet traduir un contrast sobre $p_1$ i $p_2$ pbinom{27,71,0.5)
...
\coment{El contrast és sobre les mitjanes poblacionals, no les mostrals. La hipòtesi nul·la ha de ser equivalent a ``no hi ha diferència''. No fa falta per res que les variàncies poblacionals siguin iguals: si són diferents, tan sols  cal canviar l'estadístic i el nombre de graus de llibertat de la seva distribució. I tampoc fa cap falta que les mides de les mostres siguin iguals.}
...
\coment{La definició de p-valor és ``la probabilitat d'obtenir una diferència tan o més extrema que l'obtinguda si `al món real' les dues efectivitats són iguals''. És a dir, la probabilitat que la diferència observada (o una major) s'hagi degut al pur atzar en prendre les mostres, i no a la diferència real en les efectivitats. La resta de les afirmacions no tenen res a veure amb que el p-valor sigui 0.034}
...
1. La hipòtesi nul·la és que tots dos  aparells  donen, de mitjana, els mateixos valors.\VV
1. El contrast hauria de ser unilateral. 
1. El contrast realitzat és de mostres independents. 
1. Totes les altres respostes són incorrectes. 

\coment{No fa falta saber quin és l'estadístic de contrast per concloure, emprant només el sentit comú, que no pot ser només ``el nombre de subjectes que van tenir lectura més alta amb  l'aparell  A''; segur que almenys ha de tenir en compte la mida de la mostra, no? No serà el mateix 10 de 10 que de 10 de 1000. Si encara digués ``la proporció'', podria ser, però ``el nombre'' segur que no. Per tant (a) és fals. Com que només comparam si els dos aparells donen de mitjana el mateix o no, no si un dóna de mitjana un valor superior a l'altre, el contrast és bilateral: (d) és fals.
Com que mesuram els mateixos subjectes amb tots dos aparells, el contrast és de mostres aparellades: (e) és fals. Com la mostra és de 17 individus, només podem emprar un test t si les distribucions dels valors amb els dos aparells (o almenys la seva diferència) segueixen una llei normal: (b) és falsa. I (c) és vertadera, per que la hipòtesi nul·la és ``Les mitjanes de les mesures preses amb els dos aparells són iguals''.}
...
\coment{(c) és la definició de l'error de tipus I i (b) la definició de nivell de confiança del contrast. La probabilitat d'error de tipus II és la que és u menys la potència.}
...
\coment{Es tracta de comparar dues mitjanes de variables que segueixen una distribució normal, per la qual cosa l'adequat és un test t. Com que comparam els nivells del metabolit abans i després del tractament sobre els mateixos ratolins, és de mostres aparellades.} 




**()** La prova per comparar dues mitjanes usant mostres aparellades basada en la t de Student (marcau les respostes correctes):
...
\coment{La prova per comparar dues mitjanes emprant mostres aparellades basada en la t de Student és adequada si les dues mostres provenen de variables poblacionals normals o si les dues mostres són grans (i llavors no fa falta que les distribucions siguin normals), i en cap altre cas. A més, no es pot emprar amb mostres independents (seria una altra prova específica) i no depèn de la igualtat de les variàncies (això és la prova per a mostres independents).}
...
\coment{Només podem emprar un test t  si les dues poblacions són normals o si les dues mostres són grans (no n'hi ha prou que una sigui molt gran si l'altra és petita).}
...
\coment{Com que el temps de reacció estudiat és molt asimètric a la dreta, no serà normal. Per tant, podem emprar un  test t  si les dues mostres són grans o un test no paramètric sempre. Calcular la correlació entre els temps aquí no té cap sentit, i fins i tot pot ser impossible si les dues mostres són de  mida diferent.}
...
\coment{Si les dues mostres són prou grans, i això inclou 100 o més individus, sempre podem emprar un test t per comparar les mitjanes. Si una de les mostres és petita, i això inclou menys de 20 subjectes, no serà correcte emprar el test t si les distribucions poblacionals no són normals.}
...
\coment{(a) és la definició literal de p-valor. Si teniu dubtes sobre (c), recordau que el p-valor mesura la probabilitat d'observar diferències entre proporcions, no la d'observar proporcions concretes.}
