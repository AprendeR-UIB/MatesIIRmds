...
$$
és un vector aleatori de dimensió 3. Cada cop que prenem una persona i n'anotam el BMI, el nivell de colesterol i l'edat i organitzam aquestes mesures en aquest ordre en un vector, obtenim una **realització** d'aquest vector aleatori $\underline{X}$. 

Llavors,una **mostra** de $X$ serà un conjunt de vectors amb el BMI, el nivell de colesterol i l'edat d'un grup de persones de la població. Usualment, organitzarem una mostra de $X$ per mitjà d'una taula de dades amb fileres aquests vectors i les columnes definides per les tres variables mesurades.
```
...
* En una mostra aleatòriai, totes les variables són còpies de la mateixa variable $X$ i les avaluam cada una sobre un individu diferent, obtenint una mostra de valors de $X$
...
Donades dues variables aleatòries $X,Y$ de mitjanes $\mu_X$ i $\mu_Y$, respectivament, la seva **covariància** és 
$$
...
La covariància de  $X$ i $Y$ pot prendre qualsevol valor real, i mesura el grau de variació conjunta de les variables. Si quan $X$ pren un valor gran sobre un individu, $Y$ tendeix a també prendre'l gran, i quan $X$ pren un valor petit sobre un individu, $Y$  tendeix a també prendre'l petit, la seva covariància és positiva. Si, en canvi, quan $X$ pren un valor gran sobre un individu, $Y$  tendeix a prendre'l petit i vice versa, la seva covariància és negativa. Per tant:

```{block2,type="rmdimportant"}
El signe de la covariància  reflecteix la tendència de la relació entre les variables. 
```
 
Si $X$ i $Y$ són variables independents, la seva covariància es 0, perquè en aquest cas $E(X\cdot Y) =\mu_X\mu_Y$. El recíproc és fals: dues variables aleatòries poden tener covariància 0 i no ser independents.
...
$$
\begin{array}{l}
\sigma_{X,Y)=E\big(X\cdot Y\big)-E(X)\cdot E(Y)=E\big(X\cdot Y\big)-0\cdot 2.5=E\big(X\cdot Y\big)\\
...
* La covariància és simètrica: $\sigma_{X,Y}=\sigma_{Y,X}$
...
$$
\sigma_{\underline{X})=\begin{pmatrix} \sigma_{X_1,X_1} & \sigma_{X_1,X_2} & \ldots & \sigma_{X_1,X_p}\\
...
```{block2,type="rmdimportant"}
La covariància de dos vectors només té sentit quan aquests vectors representen els valors de dues variables quantitatives sobre els mateixos individus i en el mateix ordre. En particular, per existir els dos vectors han de tenir la mateixa longitud.
```
...
$$
\widetilde{s}_{x,y}=\widetilde{s}_{y,x}
$$
i que  la covariància generalitza la variància, en el sentit que la variància d'un vector és la seva covariància amb ell mateix
$$
...
$$
\begin{array}{l}
\dfrac{1}{5}\Big((18.3-22.23)(170-205.5)+(24.4-22.23)(202-205.5)+(24.6-22.23)(215-205.5)\\
\qquad +(24.4-22.23)(218-205.5)+(22.2-22.23)(210-205.5)+(19.5-22.23)(210-205.5)\Big)=`r cov(BMI,Chol)`
\end{array}
$$
...
$$
\begin{array}{l}
\dfrac{1}{6}\Big((18.3-22.23)(170-205.5)+(24.4-22.23)(202-205.5)+(24.6-22.23)(215-205.5)\\
\qquad +(24.4-22.23)(218-205.5)+(22.2-22.23)(210-205.5)+(19.5-22.23)(210-205.5)\Big)=`r 4*cov(BMI,Chol)/5`
\end{array}
$$
...
\end{array}
$$
on cada columna representa els valors d'una certa variable $X_i$ i cada filera un individu d'una mostra de la població, de manera que l'entrada (i,j) d'aquesta taula és el valor de $X_j$ sobre l'individu $i$-èssim de la mostra. 

La **matriu de covariàncies mostrals** d'aquesta és la matriu
$$
\widetilde{{S}}=
\begin{pmatrix}  
...
```{example,multex2}
Anem a afegir a la taula de dades de l'Exemple \@ref(exm:multex1) una tercera variable amb les edats dels 6 individus representats en ella. 
...
* $r_{x,y}$ és un estimador màxim versemblant de $\rho_{X,Y}$ de les variables aleatòries poblacionals $X$ i $Y$ quan la seva distribució conjunta és normal bivariant. A més, el seu biaix que tendeix a 0. 
...
* $r_{x,y}=\pm 1$ si, i només si, existeixen  $a, b\in \mathbb{R}$ tals que $y=ax+b$, és a dir,  $y_i=ax_i+b$ per a cada $i=1,\ldots,n$. El pendent $a$ d'aquesta relación lineal té el mateix signe que $r_{x,y}$.
...
R^2=r_{x,y}^2
$$
Per tant, com més s'acosta $|r_{x,y}|$ a 1, més s'acosten els vectors $x,y$ a dependre linealment l'un de l'altre.
...
La seva **matriu de correlacions de Pearson** és la matriu
$$
\begin{pmatrix}  
...
La correlació de Spearman  de dos vectors $x$ i $y$ és la correlació de Pearson dels vectors de **rangs** $x$ i $y$. El  vector de rangs d'un vector $x$ s'obté substituïnt cada valor de $x$ per la seva posició en el vector ordenat de menor a major, i en cas d'empats la mitjana de les posicions que ocuparien tots els empats (aquests rangs ja han sortit, al test de bondat d'ajust de Kolmogorov-Smirnov). Per exemple, el vector de rangs de 
$$
x=(4,5,1,5,1,3,4,4)
$$
és
...
```{block2,type="rmdimportant"}
Si en un contrast de correlació rebutjam la hipòtesi nul·la, en particular concloem que les variables $X$ i $Y$ no són independents (perquè si fossin independents, la seva correlació seria 0).
```


No explicarem com es fa a mà aquest contrast ni quines hipòtesis han de satisfer les variables poblacionals per que el resultat sigui fiable. Simplement heu de saber que s'efectua amb la funció `cor.test`.
