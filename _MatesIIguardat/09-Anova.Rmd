...
```{block2,type="rmdnote"}
Fixau-vos que, amb la terminologia introduïda quan explicàvem el model de l'ANOVA d'1 via, cada $X_{ij}-\mu_i$ estima el **residu**, o **error**, sobre l'individu $j$-èsim del nivell $i$-èsim. Per això a $\displaystyle\sum_{i=1}^k\sum_{j=1}^{n_i} (X_{ij}-\overline{X}_{i})^2$ li diem la   *Suma de Quadrats dels Residus* o *dels Errors*.
```
...
```{block2,type="rmdnote"}
Per a que el model de l'ANOVA de blocs
$$
X_{ij}=\mu+ (\mu_{i}-\mu) +(\mu_{\bullet j}-\mu) + E_{ij}  
$$
ha de passar (aïllant) que
$$
E_{ij}=X_{ij}-\mu_{i}-\mu_{\bullet j}+\mu
$$
i aleshores $SS_E$ és la suma dels quadrats de les estimacions d'aquests errors aleatoris per als individus de la nostra mostra.
...
L'experiment es va realitzar sobre 20 femelles. Es van dividir aleatòriament en 4 subgrups de  5 exemplars cadascun. Cada grup va rebre una combinació diferent de llum i temperatura. Als 3 mesos es mesuraren els seus GSI.
...
* La variable aleatòria d'interés $X$ és el GSI d'un peix (femella) als 3 mesos
...
Com a tots els ANOVA, els factors que s'empren per classificar la població convé que siguin això, factors (o, almenys, vectors de paraules, no vectors numèrics), al *dataframe* si volem després emprar la funció `aov` per fer l'ANOVA.
...
Com als altres ANOVA, compararem la variabilitat de les mitjanes amb la variabilitat dels errors. Per això, emprarem els quadrats mitjans següents:
...
\end{array}
$$
    *Conclusió*: Hem trobat evidència estadística que el GSI mitjà és diferent segons el fotoperíode (ANOVA de blocs, p-valor 0.0003)
...
\end{array}
$$
    *Conclusió*: Hem trobat evidència estadística que no tots els GSI mitjans per a les combinacions de fotoperíode i temperatura són iguals (ANOVA de blocs, p-valor 9·10^-6^)
...
\end{array}
$$
    *Conclusió*: No hem trobat evidència estadística  que hi hagi interacció entre el fotoperíode i la temperatura (ANOVA de blocs, p-valor 0.051)
