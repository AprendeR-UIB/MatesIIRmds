...
Ara podíem emprar aquesta recta per estimar l'alçada d'un nin d'una edat concreta. Per exemple, esimam que l'alçada d'un nin de 10 anys és
$$
...
$$
Segons aquest model, l'augment de 1 g de sal consumida implica un augment mitjà de `r round(b1.sal,1)` mm Hg de pressió arterial. Amb aquest model, estimam que la tensió d'una persona que consumeix 3 g diaris de sal és
$$
...
```{block2,type="rmdimportant"}
En una regressió per mínims quadrats $R^2\leq 1$, i és 1 exactament quan tots els $e_i$ són 0, és a dir, quan $\widehat{y}_i=y_i$ per a tot $i=1,\ldots,n$. Com més gran sigui $R^2$, més bona entendrem que és la regressió lineal.
```

```{block2,type="rmdnote"}
Per si de cas no hi heu caigut, observau que $R^2\geq 0$, perquè és un quocient de quadrats. Només val 0 quan $s^2_{\widehat{y}}$, és a dir, quan tots els valors estimats $\widehat{y}_i$ són iguals.
```
...
as.numeric(glance(summary(lm(y~x1+x2+x3+x4)))[1,5]) # El p-valor
```
